...
export async function handleFakePaymentConfirmPost(
  req: NextRequest,
  ...
  if (releaseV1) {
    try {
      ...
      const reasons = error instanceof ReleaseV1ContinuityError
        ? error.reasons
        : [error instanceof Error ? error.message : String(error)];
      return NextResponse.json(
        { error: 'release_v1_authority_mismatch', reasons },
        { status: 409 },
      );
    }
  }
    ...
    try {
      ...
      if (failedTransition.count !== 1) {
        throw new ReleaseV1ContinuityError([
          'Order authority changed before fake-payment failure transition',
        ]);
      }
  ...
  let shouldTriggerGeneration: boolean;
  try {
    shouldTriggerGeneration = await prisma.$transaction(async (tx) => {
    const fresh = await tx.order.findUnique({
      where: { id: order.id },
      select: {
        ...RELEASE_V1_ORDER_AUTHORITY_SELECT,
    ...
    if (paidTransition.count === 0) {
      ...
      if (!current) {
        throw new ReleaseV1ContinuityError(['Order disappeared during paid transition']);
      }
      requireExpectedWizardProductBinding({
        order: current,
        expected: releaseBinding,
  ...
  if (shouldTriggerGeneration) {
    ...
    try {
      await triggerGeneration(
        ...
        releaseV1 ? { releaseProtocol: RELEASE_V1_PROTOCOL } : undefined,
      );
    } catch (error) {
      logger.error('Fake payment generation trigger failed', error, { orderId: order.id, paymentId });
      if (releaseV1) {
        return NextResponse.json(
          { error: 'release_v1_generation_start_failed' },
          { status: 503 },
        );
      }
    }
  ...
  return NextResponse.json({
    ok: true,
    result: 'success',
    redirectUrl: `${releaseV1 ? '/release/v1/generating' : ROUTES.generating}?orderId=${encodeURIComponent(order.id)}&accessKey=${encodeURIComponent(paymentId)}`,
  });
}
