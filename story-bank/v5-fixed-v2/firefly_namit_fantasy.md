# Story: firefly_namit_fantasy — Story Bank v5 (Hybrid: GPT-5 skeleton + Claude prose)
Generated: 2026-05-13T21:54:28.171Z
Skeleton model: gpt-5.3-chat-latest
Prose model: claude-sonnet-4-5-20250929
Total time: 80.4s
Prompt-version: v5

---
---
title: "הַחֹשֶׁךְ שֶׁבּוֹלֵעַ אור"
companionId: firefly_namit
direction: fantasy
category: GENERAL_FEARS
gender: male
pages: 20
endingType: distance

---

storyStyle: quiet nocturnal fantasy with sensory focus
metaphor: light as control vs darkness as presence
stakes: Namit burns out completely, leaving both her and child in total darkness without guidance
emotionalArc: active control → overwhelm → strain and doubt → collapse into dark → quiet steady return
quietPage: 15
heartLine: עמוד 10 — Namit hides under a leaf and cups her dim glow with her wings so the child will not see it fading
emotionalMistake: עמוד 6 — The child swats the air and tells Namit to stop lighting everything, turning away from her help
uncomfortableTruth: עמוד 12 — Even when fully lit, one shape on the wall only grows sharper and more unsettling instead of shrinking

--- Page 1 ---
הלילה מתחיל.

הקירות בחדר נעשים כהים, וצורות מתחילות להופיע. הכיסא ליד החלון נראה אחרת. המעיל על השידה מתארך.

אני עומד ומסתכל. משהו זז על הקיר בלי שום רעש.

imageDirection: soft shadows from everyday objects growing long on walls

--- Page 2 ---
פתאום — נְמית נכנסת דרך החלון.

היא גחלילית קטנה עם אור חזק מאוד. הצעיף שלה מתנפנף מאחור כשהיא טסה מהר.

"אל פַּחַד!" היא אומרת. "אני אדאג שהכל יהיה מואר!"

imageDirection: tiny firefly streak of light entering, scarf trailing

--- Page 3 ---
נמית טסה אל הכיסא בפינה.

האור שלה נופל על המעיל. עכשיו אני רואה את הכפתורים ואת השרוולים. זה רק מעיל.

היא כבר זורחת על משהו אחר.

imageDirection: coat shadow transforms into clear coat under sharp glow

--- Page 4 ---
נמית ממשיכה הלאה, אבל כשאני מסתובב אני רואה צורות חדשות בפינה השנייה.

הכנפיים שלה מרפרפות מהר. היא אומרת, "רגע, רגע, אני אגיע גם לשם!"

יש עוד פינות בחדר שעדיין חשוכות.

imageDirection: multiple faint shadows forming behind her

--- Page 5 ---
נמית טסה מהתריס לדלת, מהדלת לארון, מהארון לשולחן.

היא מאירה כל דבר רק רגע. צורות מופיעות ונעלמות. אני כמעט לא מספיק לראות.

"אל פַּחַד, אני פה!" היא אומרת.

imageDirection: motion blur of glowing paths connecting objects

--- Page 6 ---
"רגע!" אני אומר.

אני מסתיר את העיניים. האור מציק לי. החדר מרגיש כאילו הוא זז.

אני מסתובב הצידה. אני לא יכול יותר.

imageDirection: child averting eyes while light flickers nearby

--- Page 7 ---
נמית לא עוצרת.

היא טסה עוד יותר מהר. האור שלה נעשה חזק מאוד — מהבהב על התקרה, על הקירות, על כל מה שזז.

עוד צורות מופיעות מכל צד.

imageDirection: overlapping shadows, frantic glowing firefly

--- Page 8 ---
אני רואה את האור שלה מהבהב.

הכנפיים שלה זזות לאט ולא מסודר. הצעיף קופץ למעלה ולמטה.

היא מנסה להדליק הכל בבת אחת, אבל האור לא יציב.

imageDirection: unstable blinking glow, scarf fluttering wildly

--- Page 9 ---
יש צורה אחת על הקיר הרחוק.

נמית טסה אליה ומאירה אותה חזק. אבל הצורה לא נעלמת. היא נעשית יותר חדה — עם קווים ברורים וחזקים.

נמית זורחת עוד. הצורה נשארת.

imageDirection: ambiguous shape becoming more detailed and layered

--- Page 10 ---
נמית נעלמת פתאום.

אני מחפש אותה. אני רואה אותה מתחת לעלה של העציץ על השידה.

היא מקפלת את הכנפיים סביב האור שלה. מסתירה אותו. לא רוצה שאראה שהוא כבר כמעט נגמר.

imageDirection: small leaf sheltering faint light, wings wrapped inward

--- Page 11 ---
נמית יוצאת מאחורי העלה.

היא מנסה לזרוח שוב, אבל האור מהבהב ועוצר. מהבהב ועוצר.

היא לא אומרת כלום. רק מנסה.

imageDirection: hesitant flicker, uneven pulses of light

--- Page 12 ---
הצורה על הקיר עדיין שם.

נמית מאירה אותה שוב, והצורה רק נעשית יותר ברורה. יותר גדולה. יותר קרובה.

האור לא עוזר. הצורה נשארת בדיוק איפה שהיא.

imageDirection: sharp-edged shadow with fine details, looming but ordinary

--- Page 13 ---
אני עומד ומסתכל על נמית.

האור שלה רועד. היא טסה לאט, כמעט נופלת. הצורה על הקיר לא זזה.

נמית לא יכולה לתקן את זה.

imageDirection: child still, observing flicker and unmoving shadow

--- Page 14 ---
אני מושיט את היד.

אני שם אותה בין נמית לבין הקיר. האור נעצר על כף היד שלי.

הצורה מטשטשת קצת. נעשית רכה יותר.

imageDirection: hand interrupting beam, shadow softening slightly

--- Page 15 ---
האור של נמית כבה.

imageDirection: tiny silhouette of Namit with no glow

--- Page 16 ---
החדר כהה.

אני עומד עם היד מושטת. הצורה על הקיר לא חדה יותר. היא פשוט שם, מטושטשת, רגועה.

אני לא מנסה לראות אותה טוב יותר.

imageDirection: soft indistinct forms, calm stillness

--- Page 17 ---

אור קטן חוזר.

נמית זוהרת שוב — אבל לאט, חלש, כמו פנס קטן בלילה. הכנפיים שלה נחות. הצעיף לא זז.

היא לא ממהרת לשום מקום.

imageDirection: gentle pulse like a nightlight, wings relaxed

--- Page 18 ---
נמית טסה לפינה הרחוקה של החדר.

היא נשארת שם, רחוק ממני. לא מאירה כל דבר. לא ממהרת.

רק זוהרת קצת.

imageDirection: firefly hovering quietly at a distance

--- Page 19 ---
אני מרים משהו מהרצפה — משהו קטן וקר.

אני יושב איתו בחושך הרך. לא מסתכל על הקירות. לא מחפש את הצורות.

פשוט יושב.

imageDirection: child holding simple object, surrounded by soft shadows

--- Page 20 ---
החדר נשאר חצי חשוך.

נמית רחוקה, זוהרת קצת. אני לבד עם הצורות השקטות.

זה ככה עכשיו.

imageDirection: cool-toned scene, distant tiny glow, open-ended stillness

WORD_COUNT: [28, 31, 24, 29, 29, 22, 23, 25, 30, 33, 24, 26, 24, 23, 8, 28, 28, 24, 26, 18] = 503