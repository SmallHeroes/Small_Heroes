# Story: snail_sheli_bedtime — Story Bank v5 (Hybrid: GPT-5 skeleton + Claude prose)
Generated: 2026-05-13T22:06:41.104Z
Skeleton model: gpt-5.3-chat-latest
Prose model: claude-sonnet-4-5-20250929
Total time: 51.3s
Prompt-version: v5

---
---
title: "הַחֵדֶר הַקָּטָן שֶׁל שֶׁלִּי"
companionId: snail_sheli
direction: bedtime
category: SENSITIVITY_OVERWHELM
gender: male
pages: 10
endingType: resolution
worldRule: עולם של קונכיות שיש להן רגשות. החוק לא מתבטל.
---

storyStyle: gentle sensory bedtime
metaphor: a shell as both safety and a door to the world
stakes: If the child doesn't adapt, Sheli stays inside and misses small beautiful moments and the chance to be close with a friend.
emotionalArc: mid-retreat → careful approach → overwhelm and mistake → quiet grief and understanding → shared middle state
quietPage: 7
heartLine: עמוד 5 — Inside the shell, Sheli slowly pulls a tiny shiny decoration closer to himself and turns it away from the opening, dimming his own small joy.
emotionalMistake: עמוד 3 — The child taps the shell and tries to peek inside, making a sudden noise.
uncomfortableTruth: עמוד 6 — Even when everything becomes softer, Sheli still does not come out, and the child realizes he might stay inside.

--- Page 1 ---
שֶׁלִּי בפנים. הקונכייה שלו סגורה.
בחוּץ יש פרחים גדולים, ודבורה עפה מעל.
הכול רועשׁ.

imageDirection: Close-up of a small shell among big bright flowers and buzzing insects, the world oversized and noisy.

--- Page 2 ---
ילד יושב קרוב לקונכייה.
הוא לא זז.
שני מחושים קטנים יוצאים החוצה, לאט לאט.

imageDirection: Soft light, child at ground level, shell in center, two delicate antennae just emerging.

--- Page 3 ---
טיפה של מים נופלת על הקונכייה.
אוֹר חזק.
שֶׁלִּי נעלם בחזרה פנימה, והילד דופק על הקונכייה.

imageDirection: Water droplets mid-air, shell tightly closed, child's hand mid-tap, tension in the moment.

--- Page 4 ---
הילד קורא: "שֶׁלִּי?"
הוא מתקרב עוד קצת.
הקונכייה לא זזה.

imageDirection: Child leaning in, mouth open mid-call, shell unmoving, space feeling slightly too full.

--- Page 5 ---
דרך חור קטן, הילד רואה פנימה.
שֶׁלִּי מושך אליו קישׁוט קטן שזוהר, ומסובב אותו הצידה.
האוֹר שבפנים נעשה קטן יותר.

imageDirection: Cutaway-like view into shell interior with soft glowing decorations, one being turned away into shadow.

--- Page 6 ---
הילד לא זז.
הוא מניח עלה מעל הקונכייה, כדי לעשׂוֹת צל.
שֶׁלִּי לא יוצא.

imageDirection: Child gently placing a leaf like a shade over the shell, calm colors, shell unchanged.

--- Page 7 ---
הילד שם את הלחי שלו על הקונכייה.
שֶׁקֶט.

imageDirection: Quiet intimate close-up, child's cheek touching the shell, everything muted and still.

--- Page 8 ---
הילד בונה סוכה קטנה מעלים.
היד שלו נשארת קרוב.
שֶׁלִּי יוצא חצי — האנטנות שלו נוגעות זו בזו, שלושׁ פעמים.

imageDirection: Tiny green canopy, soft filtered light, Sheli half-emerged, antennae gently meeting.

--- Page 9 ---
שניהם קרובים מתחת לעלים.
לא זזים.
שֶׁלִּי לא בפנים לגמרי, לא בחוּץ לגמרי.

imageDirection: Child lying beside shell, hand near but not grabbing, calm shared space under leaves.

--- Page 10 ---
ערב יורד.
הילד ושֶׁלִּי נחים ביחד.
האנטנות נוגעות, נוגעות, נוגעות — לאט.

imageDirection: Warm evening tones, child and snail nestled together, peaceful, small movements of antennae like a lullaby.

WORD_COUNT: [15, 15, 16, 12, 21, 17, 8, 23, 14, 13] = 154