# Story: footstep_giant_bedtime — Story Bank v5 (Hybrid: GPT-5 skeleton + Claude prose)
Generated: 2026-05-14T06:11:12.597Z
Skeleton model: gpt-5.3-chat-latest
Prose model: claude-sonnet-4-5-20250929
Total time: 41.3s
Prompt-version: v5

---
---
title: "בּוּמי וְהַצְּעָדִים"
companionId: footstep_giant
direction: bedtime
category: NOISE_FEAR
gender: male
pages: 10
endingType: resolution
worldRule: עולם שבו צעדים מעירים חפצים. החוק לא מתבטל.
---

storyStyle: gentle bedtime realism with magical rules
metaphor: a body that makes sound like footsteps that wake the world
stakes: the child may run away and the companion will leave, staying alone and silent
emotionalArc: careful approach → fear grows → joy breaks control → shame and stillness → shared permission
quietPage: 7
heartLine: עמוד 5 — הוא שוכב על הקרקע, מנסה להיעלם.
emotionalMistake: עמוד 4 — הילד מכסה את האוזניים ומבקש ממנו ללכת.
uncomfortableTruth: עמוד 6 — הצחוק שלו רועם בדיוק כמו הפחד.

--- Page 1 ---
הדשא רועד. הכוסות על האבן קופצות קצת. 
מישהו בא.

imageDirection: Grass shaking, small objects jittering with each unseen step

--- Page 2 ---
בּוּמי נכנס על קצות האצבעות. הכתפיים שלו כפופות.
"סְ-לִי-חָה," הוא אומר בשקט. "הבהלתי?"

imageDirection: A very large figure on tiptoes, shoulders hunched, child watching

--- Page 3 ---
הוא מתאמץ. צועד לאט מאוד.
אבל הקרקע עושה "בום". הציפורים עפות.

imageDirection: Birds fluttering up with each careful but heavy step

--- Page 4 ---
הילד מכסה את האוזניים. מסתובב הצידה.
בּוּמי עומד במקום.

imageDirection: Child turning away, hands on ears, בּוּמי frozen mid-step

--- Page 5 ---
בּוּמי שוכב על האדמה. מנסה להיות שטוח. מנסה להיעלם.
העיניים שלו עצומות.

imageDirection: His full body pressed to earth, eyes closed, grass bent under him

--- Page 6 ---
חרק קטן זוחל לו על האף. 
בּוּמי צוחק. הצחוק רועם והכל רועד.

imageDirection: A small bug on his nose, a burst of movement rippling outward

--- Page 7 ---
הילד שם יד על הקרקע. ליד היד הגדולה.

imageDirection: Close-up of small hand touching earth near his large hand

--- Page 8 ---
הילד מקיש על האדמה. טַק טַק טַק.
מסתכל על בּוּמי.

imageDirection: Child demonstrating gentle taps, watching him

--- Page 9 ---
בּוּמי מקיש גם. טַק טַק טַק.
הקרקע עונה בשקט. לא רועדת. פשוט שקט.

imageDirection: Their hands on the earth, subtle ripples that feel gentle not scary

--- Page 10 ---
הם יושבים קרוב. בּוּמי שם את היד על הקרקע.
הילד נשען עליו.

imageDirection: Warm, intimate scene, bodies touching, both relaxed and sleepy

WORD_COUNT: [12, 18, 14, 12, 16, 13, 11, 11, 14, 11] = 132