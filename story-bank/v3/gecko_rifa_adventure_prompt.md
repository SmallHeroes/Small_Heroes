...
**PAGE COUNT IS A HARD CONTRACT.** This story MUST have EXACTLY 15 pages — no more, no less. The direction "adventure" is sold to parents as a 15-page book. Outputting a different number breaks the product. Every page from 1 to 15 must exist, in order, with both Hebrew text AND an imageDirection line.
...
5. Companion MUST speak or act on the climax pages (10-12). They don't solve it but they react.
...
**CRITICAL:** The arc shape above OVERRIDES the default page structure below. The quiet page MUST be page 11 — this is a HARD NUMBER, not a suggestion. (This page was scaled from the companion's canonical 15-page quiet position to fit a 15-page adventure story.) If the climax is internal rather than physical, follow the arcShape. The default structure is a FALLBACK for companions without an arcShape, not a law.
...
- Pages 1-3–6: Companion tries to help the child → partially fails because of their own weakness
...
- Sensory palette: Wind, colors, textures, smells, sounds of nature
- Resolution type: Child overcomes obstacle through physical action
- Companion's role: Guide, partner, co-explorer
...
2. **MIDPOINT TURN (pages 7-9):** Child discovers problem is NOT what it seemed. Changes approach.
3. **NEAR-FAILURE (early climax — page 10 or 11):** Child tries something REAL and it DOESN'T WORK.
...
- **ONE quiet page — HARD POSITION: page 11.** This is not a suggestion. The quiet page is page 11. Write it as page 11. Put "11" in the quietPage metadata. If your output has the quiet page on ANY other page number — you have failed. The quiet page: no chase, no joke, no chaos. **TWO SENTENCES MAXIMUM on this page. Not two long sentences — two SHORT ones.** One sensation. One small observation. That's it. If you wrote three sentences on this page, delete one. If you wrote a comma-joined compound sentence, split it and delete half. The illustration carries everything else. Children are not afraid of silence. Adults are afraid of silence. Trust the child.
- **DECELERATION RULE:** The page BEFORE the quiet page (page 10) must SLOW DOWN. If page 10 is full of action, chasing, or rapid dialogue, the quiet page will feel like a crash, not a breath. Page 10 should have: shorter sentences, fewer events, more space. It's the inhale before the held breath.
...
**Closing image (page 15):** intimate, concrete, warm. Body in contact with body, or hand on warm surface. Sleepy.
...
**Closing image (page 15):** companion sitting slightly apart, the residue object (feather/scale/etc) visible in environment, child nearby but not embracing companion. Soft evening or late light. Warmth AND weight.
...
**Closing image (page 15):** cool, honest, open. Companion slightly apart or facing away. Child alone with an object. The magic still visible somewhere. NOT warm.
...
3. Are the companion and child PHYSICALLY APART (not embracing) on page 15? If no → REWRITE. Move them apart.

**If direction is FANTASY:**
1. Does the world-rule that drove the story STILL show in some form on page 15? If no → REWRITE. Keep a trace of the magic.
...
1. Are the companion and child physically CLOSE on page 15? If no → REWRITE.
...
**DENSITY CAP (v6.1):** Each climax page (10-12) must stay under 55 words. If a climax page exceeds 55 words, you are describing too many actions or too many sensations. Cut. One action + one sensation + space = a page the illustrator can draw and the child can absorb.
...
- TOTAL STORY: aim for 470-560 words across 15 pages.
...
- Use the CHILD'S vocabulary for ages 5-6. A 4-year-old doesn't say "שחרר" — they say "יצא ממנו."
...
- Use words a child of age 5-6 wouldn't know
...
**HUMOR MECHANICS that work for ages 5-6:**
...
**Good:** "medium_shot from low angle: child (5-6yo boy, pajamas) crouching beside small octopus stuck in book, tentacles flailing wildly in orange-red, tiny sailor hat crooked. Cozy bedroom, warm lamp glow on left, focal point: octopus between pages."
...
0. **PAGE COUNT CHECK (MOST IMPORTANT):** Did you output EXACTLY 15 pages? Count the "--- Page N ---" markers. If you have fewer or more than 15, REWRITE until you have exactly 15. The product is sold as a 15-page adventure story.
1. Read each page aloud in Hebrew — does it FLOW naturally? Or does it sound translated? → rewrite stiff pages
2. Any page below 22 words (except the closing page 15)? → add a sensory detail or dialogue line
...
11. WORD_COUNT line uses only numbers? → fix if it has English words. The line MUST have exactly 15 comma-separated numbers in the brackets.
12. Two consecutive pages with same camera angle? → vary shots
13. Is the quiet page on page 11? → if it's on ANY other page, move it to 11
...
20. Does the story's arc follow the companion's COPING STRATEGY? → defense visible in pages 1-3-6, COLLAPSE drives pages 7-12, recovery through the strategy breaking
...
26. Does the quietPage metadata say 11? → if not → REWRITE that page to be page 11.
...
29. Is the endingType "residue"? → if mismatched → rewrite ending. Bedtime=resolution, Adventure=residue, Fantasy=distance.
...
32. Climax pages (10-12): can you replace the child with a robot and the sentences still work? → if yes, the actions are FUNCTIONAL (arranging, building, collecting). Rewrite with SENSORY verbs.
33. Page before quiet page (10): is it full of rapid action or dialogue? → slow it down. Shorter sentences, fewer events, more space. It's the inhale before silence.
...
Write the complete 15-page story now. Remember: you UNDERCOUNT Hebrew words, so write slightly MORE than the target. The page count is a HARD contract — exactly 15 pages.