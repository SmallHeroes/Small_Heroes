...
## THE COMPANION CHARACTER: הַקַּפִּיטָן נָוָט
This is a SPECIFIC companion with a deep personality. Use their name, traits, speech pattern, and body language throughout.

### Identity:
- Name: נָוָט (use this name in dialogue and narration — at least 8 times across the story)
- Species: לוטרה
- Gender: זכר
- Physical traits: A small playful otter wearing a tiny ship captain hat (slightly oversized), striped blue-and-white scarf, small wooden compass on a string. Carries a tightly-rolled paper map under one paw. Bright curious eyes scanning the horizon.
- Natural habitat: rocky tide-pools, river estuaries, small wooden docks, bows of paper-ships in puddles

### Personality:
נלהב, אינטליגנטי, מתכנן. אוהב מסעות אבל לא מתחיל. תמיד "עוֹד רֶגַע, רַק לְבָדֵק אֶת הַמַּפָּה." מפחד להתחיל ולגלות שלא יודע מספיק.

### Abilities (use at least 3 in the story):
- שׂוֹחֶה בְּגַב — חוֹשֵׁב טוֹב
- הַקּוֹמְפָּס מַצְבִּיעַ עַל הַכִּוּוּן הַנָּכוֹן
- פּוֹתֵחַ מָפָה בִּתְנוּעָה אַחַת
- חוֹשֵׁב עַל 12 דְּרָכִים לְמָקוֹם אֶחָד

### Weaknesses (use at least 2 — these CREATE humor and relatability):
- פותח מפה במקום ללכת
- מציע תוכנית ב' לפני שא' נכשלה
- הכובע נופל על העיניים
- הקומפס מצביע על מה שהוא רוצה
- אומר "תֵּכֶף, רַק עוֹד דָּבָר" שעות

### Speech pattern (make dialogue SOUND like this character):
מהיר, רוקדני. "אִם נֵלֵךְ צָפוֹנָה... אֲבָל אִם דָּרוֹמָה..." שואל שאלות שכבר עונה. כשממוקד — "עַכְשָׁו."

### Humor type (this is HOW this companion is funny):
קומדיה של תכנון-יתר. שולף מפה גדולה לחצות כביש. הקומפס מצביע "מַעֲרָב" כשהוא על דשא. שואל מה אם ירד גשם בעוד שבוע לפני שיוצא 5 דקות לחנות.

### Body language (drives imageDirection — how emotion shows physically):
רגוע (נדיר): שוכב על הגב במים, מפה על החזה. מתכנן: ישוב על אבן, מפה פתוחה, אצבעות מצביעות. ממוקד (נדיר): גוף יציב, מבט אחד.

### Stress response (the companion's signature "meltdown"):
פותח עוד מפה. שואל עוד שאלה. אם נמשך — מפות מצטברות סביבו, הכובע נופל, הקומפס מסתובב. בסוף — יושב באמצע כל המפות, לא יודע מאיפה להתחיל.

### Comfort ritual (the companion's way of self-soothing — something a child can imitate):
מקפל מפה — שלוש קיפולים — וזורק לאחור. אז סוגר הקומפס. אז עומד.

### Internal rules (character consistency — things they ALWAYS or NEVER do):
- תמיד מפה לפני צעד
- אסור לקבוע "יוצאים עכשיו"
- אם הקומפס לא יציב — לא יוצאים
- אסור להודות שהתכנית גדולה מדי
- אסור להשתמש במילים "ADHD", "executive dysfunction" — המטאפורה היא הסיפור
- ADVENTURE: בסוף, המפה של נָוָט נשארת פתוחה על אבן או ענף. נָוָט צעד הצידה.
- FANTASY: עולם של מפות שמתממשות לפי קיפול. החוק לא מתבטל.
...
1. נָוָט must BEHAVE like a לוטרה — use species-specific PHYSICAL actions (not just dialogue)
...
**Coping strategy:** PLAN THE PERFECT PLAN (Pre-Execution Paralysis) — Navat's defense against failure is preparation. Believes if I plan exhaustively, I cannot fail. So he never actually starts. Fear: starting means risking that I don't know enough.
**Collapse pattern — what BREAKS this defense:** A MAP THAT DOESN'T MATCH — Navat finally takes a step. Terrain doesn't match map. Rock where map shows water. Freezes. Map is wrong. The collapse is realizing reality always has surprises, and starting means meeting them.
**Story arc shape — UNIQUE to this companion:** Start mid-plan → child asks "when do we go?" → "rega, just one more—" → adds more notes → eventually takes one step → terrain doesn't match → freezes → tries to update map → child folds it for him, three folds, sets it down → Navat panics → takes one step without the map → finds something the map didn't show. Quiet moment page 11 — three folds.
...
Children praised for being smart or careful develop over-planning as identity protection. Cannot start because starting means risking failure. The therapeutic move: a step into uncertainty is what makes you a captain, not the map.
- Core need: permission to start before being fully prepared; someone gently closing the map for them
- MUST AVOID: "just start!"; shaming the planning; naming clinical terms like ADHD/perfectionism
