...
- Nature metaphor: **קול פנימי שחוזר ואומר "את לא מספיק" — הקול הוא ענן אפור קטן שמרחף מעל ראשה ומטפטף מילים כמו גשם** — an inner voice repeating "you're not enough" — manifested as a small gray cloud dripping words like rain.
- companionPersonality: a small flower growing from the child's shoe. Stubborn, quiet, determined. Grows toward light no matter what. Doesn't talk much — just grows. When she does speak: "I'm not done yet."
- Stakes: the word-rain is making everything soggy — her drawings bleed, her toys get heavy
- childFirstAttempt: she tries to blow the cloud away
- childFailure: blowing makes the cloud bigger (attention feeds it)
- childCost: she can't stop the cloud, but she can redirect the rain — she builds a tiny gutter from paper that channels the words into the garden
- solutionMode: **REDIRECT**
- requiredFinalAction: she folds paper → creates a channel → places it on her head → the word-rain flows down the channel → drips into the flower pot → the flower absorbs the words → grows taller → the cloud doesn't stop but its rain now feeds something beautiful → she smiles → the flower blooms with one petal for each absorbed word
- forbiddenResolution: cloud disappearing, words becoming nice, stopping the rain
