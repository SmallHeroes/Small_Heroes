...
**Category:** FOCUS (קשב, סקרנות ולמידה)
...
- Nature metaphor: **רכבת מחשבות שלא עוצרת — כל תחנה שהילדה רוצה לרדת בה, הרכבת מזנקת קדימה. "עוד לא! עוד לא! הרבה יותר מעניין בתחנה הבאה!"** — a thought-train that never stops. Every station the child wants to get off, the train accelerates. "Not yet! The next one is WAY more interesting!"
- companionPersonality: the train conductor — a squirrel with a hat who's ALSO distractible. "Next stop— oh wait, shiny! THAT stop! No wait, THIS one!" Essentially the child's brain externalized. Lovable chaos.
- Stakes: if she never gets off, she'll miss everything — seeing it all from the window but experiencing nothing
- childFirstAttempt: she pulls the emergency brake
- childFailure: the brake is connected to the horn — it just makes noise and goes faster
- childCost: she has to TRADE speed for depth — give the train something it wants (a really interesting object/thought) in exchange for stopping
- solutionMode: **TRADE**
- requiredFinalAction: she takes off her shoe (random, surprising) → dangles it out the window → the train: "what IS that? Is that... a SHOE?!" → slows to look → she says "I'll tell you about this shoe if you stop" → the train stops → she gets off → the squirrel follows → she gives the shoe a whole story (makes it up) → the train listens → she realizes: the train doesn't need to be STOPPED, it needs something interesting enough to PAUSE for → she trades a thought for a moment
- forbiddenResolution: train breaking, reaching the end of the line, someone stopping the train
