...
- Nature metaphor: **מכונת חיבוקים שנבנתה בשביל הילדה — אבל עכשיו היא מחבקת את התינוק הקטן ולא עוצרת. הילדה לוחצת על כפתורים אבל המכונה מתעלמת.** — a hug machine that was built for the child — but now it only hugs the baby and ignores her buttons.
- companionPersonality: a screwdriver-fairy (פיית מברגים) who can fix anything mechanical but is confused by emotions. "The circuit board is fine! The motors are fine! Why won't it respond to your input?! This is NOT a hardware problem!"
- Stakes: the machine is stuck in "baby mode" — it's running out of battery, and if it dies, nobody gets hugs
- childFirstAttempt: she presses every button, pulls levers, yells commands
- childFailure: the machine's screen shows "NEW USER PRIORITY" and beeps dismissively
- childCost: she has to reprogram it — not to ignore the baby, but to hug BOTH. It means learning how the machine works (frustrating, confusing)
- solutionMode: **TRICK**
- requiredFinalAction: she opens the back panel → the fairy points at wires → she finds the "SINGLE USER" switch → it's STUCK → she doesn't break it, she adds a SECOND switch next to it using a paperclip and tape → flips both → the machine stutters → two arms extend → one reaches for her, one for the baby → both get hugged → the fairy: "that's... not how engineering works but okay"
- forbiddenResolution: machine breaking, getting a new machine, the baby crying so machine stops
