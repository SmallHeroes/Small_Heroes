...
- Nature metaphor: **דלת כבדה שנועלת את החדר שבו הילדה שומרת את הדברים שהיא טובה בהם — כל ניסיון לפתוח מוסיף מנעול** — a heavy door locking the room where the child keeps the things she's good at. Each attempt to open it adds another lock.
- companionPersonality: a key-ring caterpillar with many legs, each holding a different key. Problem: none of the keys fit. Apologetic but keeps trying. "This one? No. This one? Also no. Sorry sorry sorry. Wait— this one!"
- Stakes: if the door stays locked, she'll forget what she's good at
- childFirstAttempt: she tries different keys with the caterpillar
- childFailure: every key adds a new lock — the door is now covered in locks
- childCost: raw physical effort — she has to PULL the door off its hinges instead of unlocking it. It's heavy. Her fingers slip. Her arms shake.
- solutionMode: **FORCE**
- requiredFinalAction: she grabs the door edge → PULLS → it doesn't move → the caterpillar wraps around the handle as extra grip → she pulls again → the hinges creak → one gives → she pulls harder → the door tears free → behind it: all her drawings, her games, her favorite things → she drops the door (CRASH) → the caterpillar says "...I guess we didn't need keys" → she laughs
- forbiddenResolution: finding the right key, door opening on its own, someone opening from inside
