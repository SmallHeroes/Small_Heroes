...
### Story 1
- storyStyle: **high_energy** (fast momentum, bold action, high stakes movement)
...
- Nature metaphor: **גשר שנשבר בין שני צדדים של נחל** — each side built their own half-bridge, but the halves don't match. One is wood, one is rope. One goes up, the other goes flat. They almost touch in the middle but there's a gap. Animals on both sides are stuck.
- companionPersonality: an engineering-obsessed beaver who drew SEVENTEEN blueprints (all wrong), carries a tape measure everywhere, measures things that don't need measuring ("the gap is exactly 47.3 centimeters of DISAPPOINTMENT"), refuses to admit the blueprints failed
- Stakes: the river is rising — by sunset the gap will be underwater and both sides will flood. The animals can't cross to higher ground.
- childFirstAttempt: she tries to push her side of the bridge closer to the other side
- childFailure: the bridge is anchored — pushing breaks a plank and makes the gap BIGGER. The beaver faints dramatically.
- childCost: she must tear apart her own side's "perfect" bridge to get materials that can fill the gap. Both sides lose their neat bridge and get a messy, imperfect connection — but it works.
- solutionMode: **BUILDING**
- requiredFinalAction: she yanks planks from her own bridge (it hurts to destroy something built carefully) → carries them to the gap → lays them across → they're too short → she tears rope from the other side → ties planks together with rope, extending them → splinters dig into her palms → the beaver hands her nails (finally useful) → she hammers the last plank as water touches her feet → the mismatched bridge holds → first animal crosses, then a flood of them
- forbiddenResolution: waiting for the water to go down, finding a different path, asking someone else to build it
- visibleEffect: planks ripped from neat bridge → carried to gap → rope ties them → hammering → splinters in hands → water rising → last plank placed → bridge holds → animals cross → messy but connected
