...
- Nature metaphor: **שני עצים שגדלים מאותו גזע — העץ הגדול (הילדה) רגיל לכל השמש. עכשיו העץ הקטן (התינוק) גדל ומסתיר חלק מהשמש.** — two trees growing from the same trunk. The big tree (the child) is used to all the sunlight. The little tree is growing and blocking some sun.
- companionPersonality: a bird who nests between both trees. Blunt, honest. "You're both being ridiculous. There's plenty of sky. Also, I need BOTH of you for my nest. Don't make this weird."
- Stakes: both trees are leaning away from each other — if they lean too far, the trunk will split
- childFirstAttempt: she grows taller to reach above the small tree's shadow
- childFailure: growing taller makes the trunk lean more — almost cracking
- childCost: she has to trade some of her lower branches (shade, comfort) to let the small tree get light — the branches she gives up were heavy anyway but they were familiar
- solutionMode: **TRADE**
- requiredFinalAction: she bends her lowest branch DOWN → it touches the small tree → the branch transfers → the small tree gets new shade while she gets more light up top → the trunk stops leaning → the bird settles in the junction where the branches meet → "FINALLY" → both trees stand straight → different shapes, same trunk → her canopy is lighter but higher → she can see further than before
- forbiddenResolution: small tree shrinking, more sun appearing, cutting down the small tree
