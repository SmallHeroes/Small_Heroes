...
function materializerEnvironment(environment) {
...
function runCanonicalLiveExecutionRequestMaterializerLauncher(
  options,
) {
  return (options.spawnSyncImpl ?? spawnSync)(
    ...
    {
      cwd: options.cwd,
      env: materializerEnvironment(options.env),
      shell: false,
      stdio: 'inherit',
      windowsHide: true,
    },
...
function canonicalLiveExecutionRequestMaterializerLauncherDisposition(
  result,
) {
