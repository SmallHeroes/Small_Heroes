...
async function main(): Promise<void> {
  ...
  console.log(`Model: ${STYLE_01_GPT_MODEL} | Ref config: ${resolveStyle01RefBudgetConfig()}`);
  ...
  for (const page of pagesToRender) {
    ...
    const cost = estimateGptImage2CostUsd(meta?.style01Meta?.usage as Record<string, unknown> | undefined);
    totalEstimatedCostUsd += cost.estimatedCostUsd ?? 0;
  ...
  console.log(`Est. cost: $${totalEstimatedCostUsd.toFixed(2)}`);
}
