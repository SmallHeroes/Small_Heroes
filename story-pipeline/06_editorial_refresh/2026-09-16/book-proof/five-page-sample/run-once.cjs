...
const output='outputs/panda-five-page-sample-20260919';
const logs=path.join(repo,'outputs/panda-five-page-execution-20260919');
...
function main(){
  ...
  const args=['--import','tsx','--require','./scripts/shims/register-server-only.cjs','scripts/run-owner-book-draft.ts',
    'outputs/panda-five-page-input-20260919/config.json','--sample','--key-env-file',process.argv[2]];
  ...
  write('before.json',before);write('invocation.json',{head,executable:process.execPath,args,startedAt:new Date().toISOString(),
    scope:'owner-requested local unaccepted sample; not canonical candidate promotion',pages:[1,2,3,4,5],imageBudgetUsd:3.5,qaBudgetUsd:6});
