...
const medium=process.argv.includes('--medium'),quality=medium?'medium':'low';
const argv=process.argv.slice(2).filter(a=>a!=='--medium');
const stem=medium?'panda-sunburst-medium':'panda-sunburst-comparison';
const input=`outputs/${stem}-input-20260923`;
const output=`outputs/${stem}-sample-20260923`;
const logs=`outputs/${stem}-execution-20260923`;
const baseline=medium?'outputs/panda-sunburst-comparison-sample-20260923':'outputs/panda-sequence-pair-sample-20260923';
const imageCapUsd=medium?.5:1,qaCapUsd=medium?1.5:1.8;
const previous=['outputs/panda-five-page-sample-20260919','outputs/panda-five-page-selected-sample-20260919',
  'outputs/panda-sequence-pair-sample-20260923','outputs/panda-sequence-pair-repair-sample-20260923',
  ...(medium?['outputs/panda-sunburst-comparison-sample-20260923']:[])];
...
function cost(){const rows=previous.map(root=>({root,accountedUpperUsd:accounted(root)}));
  const prior=rows.reduce((s,r)=>s+r.accountedUpperUsd,0);assert(prior+imageCapUsd+qaCapUsd<=9.5+1e-8,'aggregate_fence');
  return {rows,priorAccountedUpperUsd:prior,imageCapUsd,qaCapUsd,aggregateFenceUsd:9.5,maximumWithNewCapsUsd:prior+imageCapUsd+qaCapUsd};}
function expected(){return {...read('outputs/panda-sequence-pair-input-20260923/config.json'),
  imageModel:model,outputDir:output,imageBudgetUsd:imageCapUsd,qaBudgetUsd:qaCapUsd,
  ...(medium?{imageQuality:quality,samplePages:[1]}:{})};}
function verifyInput(){const config=read(input+'/config.json');assert.deepEqual(config,expected());
  ...
  assert.equal(loaded.imageQuality,quality);
...
function snapshot(){
  ...
  for(const stem of ['panda-sequence-pair','panda-sequence-pair-repair',...(medium?['panda-sunburst-comparison']:[])])
...
async function main(){
  ...
  if(mode==='--prepare'){
    ...
    verifyInput();console.log(JSON.stringify({status:'prepared',model,quality,baselinePromptIdentical:true,cost:cost(),preservedFiles:snapshot().length,providerCalls:0}));return;
  }
  if(mode==='--check'){
  ...
  if(mode==='--run'){
    assert(argv.length===2&&argv[1],'key_file_required');
    ...
    const keyFile=argv[1];assert(require('dotenv').parse(fs.readFileSync(keyFile)).OPENAI_API_KEY?.trim(),'existing_key_missing');
    ...
    write(logs+'/invocation.json',{head:git('rev-parse','HEAD'),args,startedAt:new Date().toISOString(),budget,
      configSha:previewSha(fs.readFileSync(path.join(repo,input,'config.json'))),model,quality,
      scope:medium?'one quality-only diagnostic page; no repair or acceptance':'one model-only diagnostic pair; no repair or acceptance'});
  ...
  if(mode==='--report'){
    ...
    assert.equal(read(output+'/identity.json').quality,quality);
    const manifest=read(output+'/sample-manifest.json');assert.deepEqual(manifest.samplePages,config.samplePages);
    ...
    for(const row of manifest.results){
      if(row.history?.length){assert.equal(row.history.length,1);const {candidate,review}=row.history[0];
        ...
        assert.equal(record.fingerprint,previewSha(JSON.stringify({version:'owner-book-draft/v1',model,quality,size:'1024x1536',prompt,refs:request.references})));
    ...
    const report={model,quality,status:manifest.status,baselinePromptAndReferencesIdentical:true,execution,
    ...
    if(process.argv.includes('--write')){
      ...
      const panels=[{label:`${medium?'Sunburst':'Image 2'} LOW · previous HELD baseline`,src:`../${path.basename(baseline)}/page-01.png`},
        ...manifest.results.filter(r=>r.candidate).map(r=>({label:`Sunburst ${quality.toUpperCase()} · page ${r.pageNumber} · ${r.status}`,src:r.candidate.imageName}))];
      const html=panels.map(p=>`<figure><figcaption>${p.label}</figcaption><img src="${p.src}" alt="${p.label}"></figure>`).join('');
      write(output+'/index.html',`<!doctype html><html lang="he" dir="rtl"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>השוואת איור</title><style>body{margin:24px;background:#f4f0e8;color:#24372f;font:18px Arial}main{display:flex;flex-wrap:wrap;gap:20px}figure{margin:0;flex:1 1 420px}img{width:100%;max-height:85vh;object-fit:contain}figcaption{padding:12px;direction:ltr}pre{direction:ltr;text-align:left;white-space:pre-wrap;overflow-wrap:anywhere}</style><h1>השוואת איור · הפנדה</h1><p>אותם פרומפטים ועוגנים. ${medium?'אותו מודל, איכות LOW מול MEDIUM.':'אותה איכות LOW, מודל שונה.'} בדיקה מקומית, לא אישור מוצר. מצב: ${report.status}.</p><p>אומדן שימוש בסבב $${report.knownUsageEstimateUsd.toFixed(4)}; לא חשבונית. עמודים שלא הושלמו: ${report.unassessed.join(', ')||'אין'}.</p><main>${html}</main><details><summary>ראיות QA ועלות</summary><pre>${JSON.stringify(report,null,2).replace(/&/g,'&amp;').replace(/</g,'&lt;')}</pre></details></html>`);
    }
