# Story Bank v3 — Master Generation Prompt
# Copy everything below the line and paste into ChatGPT / API
# Replace ${...} variables with actual companion data before sending
# ─────────────────────────────────────────────────────────────
...
**Coping strategy:** ${c.copingStrategy}
**Collapse pattern — what BREAKS this defense:** ${c.collapsePattern}
**Story arc shape — UNIQUE to this companion:** ${c.arcShape}

**CRITICAL:** The arc shape above OVERRIDES the default page structure below. If the companion's arc says the quiet moment comes on page 4-5, put it there — NOT on page 7-9. If the climax is internal rather than physical, follow that. The default structure is a FALLBACK for companions without an arcShape, not a law.
...
- MUST AVOID: ${psych.avoid → semicolon-separated}
...
- **ONE quiet page — HARD POSITION: page {quietPagePosition}.** This is not a suggestion. The quiet page is page {quietPagePosition}. Write it as page {quietPagePosition}. Put "{quietPagePosition}" in the quietPage metadata. If your output has the quiet page on ANY other page number — you have failed. The quiet page: no chase, no joke, no chaos. **TWO SENTENCES MAXIMUM on this page. Not two long sentences — two SHORT ones.** One sensation. One small observation. That's it. If you wrote three sentences on this page, delete one. If you wrote a comma-joined compound sentence, split it and delete half. The illustration carries everything else. Children are not afraid of silence. Adults are afraid of silence. Trust the child.
- **DECELERATION RULE:** The page BEFORE the quiet page (page {quietPagePosition - 1}) must SLOW DOWN. If page {quietPagePosition - 1} is full of action, chasing, or rapid dialogue, the quiet page will feel like a crash, not a breath. Page {quietPagePosition - 1} should have: shorter sentences, fewer events, more space. It's the inhale before the held breath.
...
Use this companion's sensory world for the PALETTE of sensations to choose from: ${c.sensoryPalette}
...
**Good:** "medium_shot from low angle: child (5yo boy, pajamas) crouching beside small octopus stuck in book, tentacles flailing wildly in orange-red, tiny sailor hat crooked. Cozy bedroom, warm lamp glow on left, focal point: octopus between pages."
...
13. Is the quiet page on page {quietPagePosition}? → if it's on ANY other page, move it to {quietPagePosition}
...
26. Does the quietPage metadata say {quietPagePosition}? → if not → REWRITE that page to be page {quietPagePosition}.
...
33. Page before quiet page ({quietPagePosition - 1}): is it full of rapid action or dialogue? → slow it down. Shorter sentences, fewer events, more space. It's the inhale before silence.
