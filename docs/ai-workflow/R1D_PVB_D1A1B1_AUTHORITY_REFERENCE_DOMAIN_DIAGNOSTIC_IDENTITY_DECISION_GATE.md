...
4. **Deterministic normalization and bounded persistence.** Approve canonical sorting and deduplication by issue code plus locator. `totalCount` is the number of unique typed issues after normalization/deduplication and before the 128-item persistence cap. Persist that exact safe-integer `totalCount`, at most 128 normalized items, and an explicit `truncated` flag; retain the existing bounded terminal `diagnosticCount`/`diagnosticCodes` as a summary. Unknown/extra keys, invalid integers, and unrecognized enums fail closed.

5. **Visual Contract-specific terminal detail.** Approve leaving the shared `AuthoringTerminalFailure` and production Blueprint receipt shape unchanged. Define a Visual Contract-specific failure extension with required `authorityReferenceDiagnostics`, populated only for `draft_authority_reference_domain_invalid` and `null` for every other Visual Contract terminal. Enforce that extended shape with a Visual Contract-specific exact-key validator; do not widen or weaken shared `authoringTerminalFailureIsValid`. Readiness copies the full Visual Contract failure without reinterpretation; attempt-level generic diagnostic codes remain a bounded summary, and production Blueprint receipt v4 remains unchanged.
...
- one deterministic unit case for every catalog identity and every locator field-role, including `subject`, `object`, `spatialEffect.target`, `spatialConstraint.target`, and `safetyConstraints.target`;
