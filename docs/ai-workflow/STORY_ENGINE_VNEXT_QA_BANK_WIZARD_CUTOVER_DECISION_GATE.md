...
3. **One bounded text call per story.** Use `gpt-5.6-sol` through the Responses API, Standard service tier, `store:false`, no transport retry, no fallback, one structured-output call per story, and a hard corpus cap of USD 5.00. The existing credential is read only by the launcher and inherited only by the private child. One application-level replacement is allowed only when a completed structured result fails deterministic normalization/validation; completed records are imported by digest instead of being called again, and the unaccounted failed call receives a conservative full-reservation cost.
