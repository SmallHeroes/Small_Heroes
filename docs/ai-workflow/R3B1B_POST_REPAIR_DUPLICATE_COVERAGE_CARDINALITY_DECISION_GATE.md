...
Status: **ZERO-COST DECISION GATE PREPARED; IMPLEMENTATION GO PENDING — NO
CODE CHANGE, RETRY, PROVIDER CALL OR DOWNSTREAM AUTHORITY**
...
- no fallback, transport retry or additional cleanup authority is introduced;
...
stop with the existing exhaustion behavior. The terminal-reference cleanup
call remains unavailable to this route.
...
8. new requests bind the new routing authority and cannot downgrade to legacy
   behavior;
9. no story-specific branch, fallback, transport retry, extra call, cleanup
   reallocation, threshold change or downstream authority is introduced; and
10. documentation and receipts state observed results without self-awarding
...
6. prove unchanged standard/cleanup call ceilings, no fallback and no transport
   retry;
...
consume one of the already-budgeted standard repair calls. It must not add a
call or raise the projected/hard ceilings. The exact future P1-A1 call and spend
authority must be issued anew after implementation QA.
...
- unchanged seven-standard-plus-closed-cleanup ceilings, no provider validation;
  and
...
> ללא retry, ספק או downstream, ולעצור אחרי commit ו-handoff ל-Claude Code.
...
5. call count, cleanup eligibility, fallback or transport behavior changed;
...
7. **Claude target?** Graph feasibility, authority closure, budget invariants and
   old/new replay isolation.
...
**Gate disposition:** the zero-cost investigation and Decision Gate are
prepared. STOP. No code change, retry, provider call or downstream work is
authorized until Guy explicitly approves the implementation package above.
