...
The pre-correction redeploy is `dpl_3pVveNJVbkBU4TEmYLu7ASm5kGZY`; it reached Ready and `qa.smallheroes.co.il` resolved to it, proving the stable branch-domain routing. Correction deployment `dpl_HcTWRpPWaztv7Yq3HpPX9wRXru2x` then reached Ready for exact commit `d62f0c573df852005c0953a7e6cc4f68a85bcdb5` and received both the branch alias and `qa.smallheroes.co.il`.

Authenticated verification through the stable domain returned exactly six categories and eighteen direction slots. All eighteen were `storyReady:true`, `qaAuthoringReady:true`, `selectable:true`, `availabilityStage:qa_ready_for_blueprint_authoring`, and carried eighteen unique candidate digests; exactly zero were `productionRenderQualified`. The tracked Bunny/Bar Reader endpoint independently returned the expected fixture id and title, eight pages, eight image pages, eight distinct static URLs, `tracked-qa-reader-fixture/v1`, and source-render HEAD `9a58a967e0b6486af4a7897c67314d8f199cd17d`.
