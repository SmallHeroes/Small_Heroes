...
**Updated:** 2026-08-12
**Maintainer:** Codex
**Working branch:** `codex/story-bank-next-generation-creative-briefs` in `C:\Users\guyna\.codex\worktrees\1602\Small_Heroes`, based exactly on reviewed local foundation commit `5368bce73978e159fe1d64ab2662e8e734cfa29b`. This branch/worktree is the sole writer for the creative-brief milestone and remains local and unpushed.

## STORY BANK NEXT-GENERATION CREATIVE BRIEFS — independent technical PASS / Guy creative review pending

Guy replaced the manual-spine-per-story workflow on 2026-08-12: ChatGPT should write complete stories from a strong shared structure, while human creative work supplies one distinct brief for every story. This milestone implements that decision without generating prose or touching runtime.

- `story-pipeline/03_story_briefs/` now contains one shared ChatGPT writer contract and exactly **18** structured creative briefs: six current MVP companions × bedtime/adventure/fantasy. Each companion set contains one 8-page bedtime, one 12-page adventure, and one 16-page fantasy brief mapped exactly to `MVP_STORY_MATRIX`.
- A brief locks premise, visible child want, physical problem/play rule, meaningful set-piece chain, causal movement, companion wrong help, three distinct comic escalations, two attempts, child discovery/climax, visible payoff, energy landing, bounded props/cast, reread hooks, safety, model freedom, and closed old-story anti-copy disclosures. It does not contain page prose, `imageDirection`, or a hand-authored page spine.
- The shared writer contract tells ChatGPT to return one complete current-format staging draft with exact 8/12/16 pages, current `{{childName}}` and `{boy-form|girl-form}` personalization shape, direction word bands, natural oral Hebrew, state-changing page turns, lean English `imageDirection` lines, child-owned climax, companion-specific causal comedy, and fail-closed `BRIEF_CONFLICT` handling. It explicitly grants no approval, bank, runtime, or render authority.
- The prior Koko/bedtime choice B survives as the brief `תחנת האוטובוס שקמה והלכה`; its rejected manually authored eight-page spine was removed and is not an input. The premise tournament is now an optional ideation tool rather than a mandatory production stage.
- `STORY_BRIEF_REVIEW_HE.md` gives Guy one compact Hebrew review table covering all 18 premises, child climax actions, and location movement. All brief records remain `draft_for_guy_review`; no prose is authorized by their existence.
- Claude Code's read-only review of exact range `5368bce73978e159fe1d64ab2662e8e734cfa29b..2304510db8966efbeeaebaf0b284cdbd502c57f4` issued **PASS — 0 blocker / 0 major / 2 minor** with topology PASS. Codex accepted both minors: the briefs could contaminate prose with slash/bare-masculine child grammar, and this section abbreviated the package hash incorrectly.
- The QA correction normalized all 18 JSON briefs to `{{childName}}` plus distinct `{boy-form|girl-form}` chips where Hebrew grammar differs, gave supporting characters explicit non-placeholder wording, and made the Hebrew review table gender-neutral. Its guards rejected slash forms, generic singular child aliases, identical pipe variants, missing discovery/climax chips, and more than six high-level causal movements; the prior vacuous empty-directory assertion was replaced by direct forbidden-spine-key inspection.
- Claude Code's read-only re-gate of correction range `2304510db8966efbeeaebaf0b284cdbd502c57f4..ce66689c4ed50069454faba5c886c28f80356367` and combined range `5368bce73978e159fe1d64ab2662e8e734cfa29b..ce66689c4ed50069454faba5c886c28f80356367` issued **PASS — 0 blocker / 0 major / 2 new minor** and closed both original findings. Codex accepted both new guard-design minors: a mandatory chip in discovery/climax rejected naturally invariant Hebrew and forced three neutral verbs to become gendered, while chip presence alone did not prove grammatical correctness, order, or coverage.
- This remediation restores those three natural invariant discovery phrasings and removes the vacuous mandatory-chip rule. Deterministic validation now rejects Hebrew slash and parenthetical suffix forms, generic singular child aliases, raw `childName`, identical pipe variants, generated-prose markers, unknown structured keys recursively across all object-bearing brief layers, and more than six high-level causal movements. It intentionally validates syntax and structure only; Hebrew morphology, male/female order, and semantic coverage remain explicit human language-QA responsibilities.
- Post-remediation deterministic validation passed **3 files / 20 tests**. It proves exact matrix coverage, page counts, schema/field completeness, meaningful-location minima, five-or-six-step causal bounds, bounded props/cast, exactly three causal comic escalations and two attempts, companion markers, child-owned discovery/climax, no encoded manual spine, acceptance of neutral child grammar, rejection of malformed placeholder/structure shapes, unique IDs/titles/mechanics/hooks/climaxes/payoffs across all 18, old-plot/therapy exclusion from positive creative fields, review-document coverage, and the shared fail-closed writer contract. The corpus contains chips in **15/18** discoveries and **18/18** climaxes, with **221** total chip occurrences and **94** distinct pairs; chip absence is valid where the Hebrew is invariant. Claude manually verified the pre-remediation 95 distinct pairs and all 21 chip-free `{{childName}}` fields; the three restored neutral phrases are the subset it identified as naturally invariant.
- `npx --no-install tsc --noEmit` passed after remediation. The completed literal `npm run check` rerun had canonical inventory **289** files (**270** ordinary, **19** resource-intensive): resource-intensive passed at two workers in `96148 ms`; ordinary completed in `32944 ms` with exactly the six established missing ignored-output fixture assertions and no seventh assertion. Exit was assertion-driven with `signal:null`, `launchErrorCode:null`, valid diagnostic protocol, and no timeout, RPC/IPC, reporter, launch, signal, teardown, or protocol failure. Repository/release status remains HOLD only on those six existing fixtures.
- Claude Code independently re-gated exact remediation range `ce66689c4ed50069454faba5c886c28f80356367..42e23414e1680141c43b4367a5dd1bf6aadd7cd4` and combined range `5368bce73978e159fe1d64ab2662e8e734cfa29b..42e23414e1680141c43b4367a5dd1bf6aadd7cd4`, returning topology **PASS** and technical **PASS — 0 blocker / 0 major / 0 minor**. Both prior guard-design minors are independently closed. Claude reproduced **3 files / 20 tests**, TypeScript, exact allowlist parity, all four corpus counts, hashes, neutral discovery and climax acceptance, hostile mutation rejection, and unchanged excluded surfaces. It did not rerun the literal repository check, so those timings and six-fixture results remain Codex execution evidence. This is Claude's verdict; Codex does not self-award independent PASS.
- Claude retained two no-occurrence advisories without findings: raw `childName` matching is case-sensitive, and identical-chip comparison does not trim whitespace. Neither affects the reviewed corpus. They are deliberately not changed after PASS; any future hardening would be a separately reviewed code milestone. Swapped chip order remains an explicit human language-QA responsibility.
- Approved V3/V5 bank sources, loader, product matrix, packages, lockfile, runtime prompts, orders, candidates, Blueprint, Wizard, reader, and render paths are unchanged. Package SHA-256 identities remain `7DF1D93BCD93E7CE577525627048584096A04C110FC3D0E9D21436242308993D` and `BF7932428AC1BC2CB8885E83A21F231486F35EA36820381B7D1763A77BA03D59`.
- No credential access, provider/model/network/pricing call, story generation, bank write, storage/database action, image/audio/render, publication, promotion, activation, deployment, or push occurred. External cost is `$0`.
- Durable records: `docs/ai-workflow/STORY_BANK_NEXT_GENERATION_CREATIVE_BRIEFS_DECISION_GATE.md` and `docs/ai-workflow/STORY_BANK_NEXT_GENERATION_CREATIVE_BRIEFS_IMPLEMENTATION_EVIDENCE.md`.
- **Next action:** after this documentation-only closeout commit, Guy reviews the 18 premises and accepts/revises briefs. Push remains separately authorized. Only a later explicitly approved execution may send one accepted brief plus its companion bible and the writer contract to ChatGPT for a single staging draft.

## STORY BANK NEXT-GENERATION FOUNDATIONS — independent technical PASS / creative workflow superseded by brief catalog
...
- The stale pipeline tracker and old prose template are fenced. V3/V5 prose and old premise files are historical anti-copy evidence only and cannot seed new plots. The legacy full-prose prompt remains on HOLD and is replaced for staging drafts by the shared writer contract plus one accepted brief.
- The Koko/bedtime pilot contains exactly 12 complete premise candidates across nine recorded premise families and exactly three materially different finalists: `היום שנכנס לתיק של קים`, `תחנת האוטובוס שקמה והלכה`, and `תחנת הדלתות של בין הערביים`. Guy selected B. Codex did not select a winner. The later manual-spine method was rejected; B now lives as a compact creative brief.
...
- Claude Code independently reviewed exact range `22857995df578d366809226a5d693cd783a10ed7..5368bce73978e159fe1d64ab2662e8e734cfa29b` and returned technical PASS with one MINOR: finalist A reused the old remembered-color matching/blue-solution shape without sufficient disclosure. The finding was accepted; A was withdrawn from recommendation and Guy selected B. Full prose, bank integration, visual-contract rebuild, and any render remain separately gated.
