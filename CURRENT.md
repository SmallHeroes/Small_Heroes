...
**Updated:** 2026-07-27
**Maintainer:** Codex
**Working branch:** `codex/r1d-pvb-d1a1a-live-authoring-boundary` in `C:\Users\guyna\.codex\worktrees\dc3a\Small_Heroes`, based exactly on immutable D1A0 source SHA `031bcb6aa08d4a9ee7cf527bb051970efc3e96d1`. D1A1A implementation commit `127335e7` is local and unpushed. D1A1A has local engineering evidence but has not yet received Claude Code's independent first-pass review, so no technical PASS is claimed. D1A0's independently re-gated PASS remains base evidence only. D1A1B/D1A1C, D0/PVB-C/PVB-B/PVB-A, Fox Board, main, Style01, and every other worktree remain read-only evidence.

## R1D-PVB-D1A1A — Canonical Live Authoring Boundary

Guy approved the nine R1D-PVB-D1A1 architectural decisions and authorized D1A1A only. Commit `127335e7` implements the zero-cost executable/provider-evidence boundary required before any real D1A authoring invocation. It performed no live authoring and created no real candidate, reconciliation, approval, Blueprint, Board, package, render, publication, production, or deployment authority.
...
- The launcher passes the existing process environment to the OS without enumerating or copying credential-bearing values in launcher code, temporarily adds only its capability key, and restores that key even on child failure. The private entry consumes the capability before the CLI core loads. Direct private-entry and direct core subprocesses fail before CLI/provider work.
...
- `preflight` accepts no Story Source, output, credential, model, price, or write authority. It imports the exact future adapter/runner graph under independent throwing raw-network plus credential/write sentinels. `TSX_DISABLE_CACHE=1` isolates runtime compiler caching from the write boundary. Output reports only checked export/version labels and explicitly says credentials, provider connectivity/configuration, prices, and billing were not checked.
...
- Before credential reachability, the adapter independently enforces OpenAI / Responses / `gpt-5.6-sol` / service tier `default` / reasoning `medium` / strict `vc-draft-schema/v6` / tools disabled / no fallback / transport retries `0` / timeout `1,200,000 ms` / input ceiling `64,000` / page-derived `32,000..64,000` output ceiling.
- Adapter construction performs no credential or provider work. Only `OPENAI_API_KEY` may be read from the existing process environment, inside `call`, after the lifecycle's exact source/snapshot/request/schema/price/spend/input/options gates and the adapter's duplicate policy check pass. No env file or alternate credential source exists.
...
- Raw prompts, raw output, raw provider payload/exception/stack, credential/header/environment values, and unknown request input fields are excluded from persisted request/receipt/readiness evidence. Existing unbranded injected-provider failure receipt behavior is preserved; the stricter evidence contract activates only for the canonical branded adapter.

### Exact live authority, cost fence, and immutable evidence

- The future live runner consumes path/data inputs, rebuilds current Story Source authority, validates the supplied content-addressed snapshot, rebuilds and compares a separately authored `mode=live` request, and refuses to promote a D1A0 preflight request in place. Source/request/topology/provider/endpoint/model/tier/reasoning/schema/prompt/pricing/cost/retry/timeout/tools/fallback/token/call/repair mutations stop before credential/provider reachability.
- The approved fence remains at most three application calls, at most two semantic repairs, zero transport retries, `$5.00` hard ceiling, and `$4.884` maximum current reserved exposure. D1A1A did not verify prices, spend, or contact any external surface. Any future price mismatch after separately authorized official verification stops D1A1B and requires new authority.
- A handled invocation persists sanitized content-addressed request evidence, receipt, and readiness. An exact approved request uses the existing request artifact; a rejected request uses a separately versioned sanitized rejection envelope that drops unknown fields. Identical bytes are idempotent, different bytes collide closed, and a candidate is written only after complete compile plus per-page structured action authority.
...
- Forced process termination can occur after provider completion but before an in-memory sanitized receipt is durably persisted. The runner exposes this limitation verbatim and grants no rerun authority; provider-side evidence would require reconciliation.

### Validation evidence and remaining gate

- Canonical launcher/adapter/runner suites: **2 files / 82 tests PASS**. Coverage includes exact dependency/shim ordering, Windows paths with spaces and POSIX-shaped paths, private/core refusal, parser exclusivity, sentinel positive controls/restoration/overlap, real sentineled import subprocess, delayed credential reachability, exact request body/options, complete usage/status mapping, invalid/stale authority tables, zero/one/two repairs, exhaustion, `$4.884` reservation, sanitization, request/receipt/readiness/candidate idempotence/collisions, downstream separation, and a real handled-failure child exit.
- Final focused plus compatibility set: **10 files / 246 tests PASS**. This includes D1A0 source-authority/lifecycle, legacy fail-closed subprocesses, production lifecycle foundation, compiler/template/repair suites, exact authoring zero-retry behavior, and the unrelated generic three-retry default.
...
- Literal `npm run check`: TypeScript **PASS**; Vitest **263 files total — 242 passed, 16 skipped, 5 failed; 2,834 tests total — 2,763 passed, 65 skipped, 6 failed**. The six failures are exactly D1A0's independently established absent ignored-output baseline in `child-lexicon-ages-5-8.spec.ts`, `momentum-gate-koko.spec.ts`, `page-entity-qa.spec.ts`, `set-appearance-ref-budget.spec.ts`, and two `story-read-back-validation.spec.ts` cases. Relative to D1A0's recorded 261-file/2,752-test run, D1A1A adds exactly 2 passing files and 82 passing tests. No new/changed test failed; no ignored fixture was copied or fabricated.
...
- Cost, generation count, and external calls are exactly zero. No internet/documentation/pricing lookup, fetch/network, real credential/env-file access, provider/model/LLM, image/audio/render/Vision, Supabase/storage/database, Board, real Semantic Reconciliation/approval, Blueprint, package/publication/promotion, production activation, deployment, PR, push, Story Source edit, or unrelated cleanup occurred.
- Rollback is a focused revert of the local D1A1A commits; D1A1A created no external state. The next action is Claude Code first-pass **READ-ONLY** adversarial review of the exact immutable D1A1A range. D1A1B and D1A1C remain unauthorized.
