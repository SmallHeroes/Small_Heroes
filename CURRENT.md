...
**Updated:** 2026-07-27
**Maintainer:** Codex
**Working branch:** `codex/r1d-pvb-d1a1a-live-authoring-boundary` in `C:\Users\guyna\.codex\worktrees\dc3a\Small_Heroes`, based exactly on immutable D1A0 source SHA `031bcb6aa08d4a9ee7cf527bb051970efc3e96d1`. Initial D1A1A commits `127335e7` and `e069f19e` are local and unpushed. Claude Code reviewed `031bcb6a..e069f19e` and returned **HOLD** with 2 MAJOR and 4 MINOR findings. Corrective code/test commit `e8a01182` accepts and addresses all six findings plus the authorized pre-spend hardening; it is locally green and awaits independent READ-ONLY re-gate, so no technical PASS is claimed. D1A0's independently re-gated PASS remains base evidence only. D1A1B/D1A1C, D0/PVB-C/PVB-B/PVB-A, Fox Board, main, Style01, and every other worktree remain read-only evidence.

## R1D-PVB-D1A1A — Canonical Live Authoring Boundary

Guy approved the nine R1D-PVB-D1A1 architectural decisions and authorized D1A1A only. Commit `127335e7` implemented the initial zero-cost executable/provider-evidence boundary, `e069f19e` recorded its handoff, and Claude Code returned HOLD for that exact range. Corrective milestone `R1D-PVB-D1A1A-QA-FIX` remains inside the same zero-cost scope. Commit `e8a01182` fixes both MAJORs, all four MINORs, and the specifically authorized provider-storage/evidence-brand/prompt-authority advisory surfaces. Neither the initial milestone nor the correction performed live authoring or created real candidate, reconciliation, approval, Blueprint, Board, package, render, publication, production, or deployment authority.

### Independent QA HOLD and accepted correction
...
- Corrected canonical launcher/adapter/runner suites: **2 files / 94 tests PASS**. New adversarial coverage includes installed-SDK environment-default bypass, exact destination/redirect/header guard, `store:false`, child environment filtering, same-path sentinel arming, output traversal/symlink/unwritable rejection, exact authority/receipt write order, stable rejected reasons, canonical key-order/Unicode bytes, 12/13-page behavior, exact evidence-brand enforcement, and prompt digest/version authority.
- D1A0/compiler/pipeline compatibility: **10 files / 210 tests PASS**, including source authority, legacy fail-closed subprocesses, production lifecycle foundation, compiler source guard/template/repair behavior, exact authoring zero-retry behavior, and the unrelated generic three-retry default. An additional compiler/package surface passed **7 files / 115 tests**. Across those non-overlapping validation sets: **17 files / 325 tests PASS**.
...
- Literal `npm run check`: TypeScript **PASS**; Vitest reproduced only the six established ignored-output baseline failures in the same five files: `child-lexicon-ages-5-8.spec.ts`, `momentum-gate-koko.spec.ts`, `page-entity-qa.spec.ts`, `set-appearance-ref-budget.spec.ts`, and two `story-read-back-validation.spec.ts` cases. With the 12 corrective tests added to the prior 263-file/2,834-test state, the resulting totals are **263 files — 242 passed, 16 skipped, 5 failed; 2,846 tests — 2,775 passed, 65 skipped, 6 failed**. No new/changed test failed; no ignored fixture was copied or fabricated.
...
- Cost, generation count, and external calls are exactly zero. All fetch surfaces in tests were throwing or injected local transports. No internet/documentation/pricing lookup, real fetch/network, real credential/env-file access, provider/model/LLM, image/audio/render/Vision, Supabase/storage/database, Board, real Semantic Reconciliation/approval, Blueprint, package/publication/promotion, production activation, deployment, PR, push, Story Source edit, or unrelated cleanup occurred.
- Rollback is a focused revert of the local QA correction commits after `e069f19e`; D1A1A created no external state. The next action is Claude Code **READ-ONLY re-gate** of the exact correction and combined immutable ranges from the final clean HEAD. D1A1B and D1A1C remain unauthorized.
...
- `visual-contract-authoring-request/v2` fixes OpenAI, Responses API, exact `gpt-5.6-sol`, standard/default tier, reasoning `medium`, strict `vc-draft-schema/v6`, tools disabled, `noFallback:true`, `transportRetries:0`, 20-minute timeout, 64,000 conservative input-token ceiling, page-derived output ceiling, initial call plus at most two repair calls, and a $5.00 hard ceiling.
