...
**Updated:** 2026-08-05
**Maintainer:** Codex
**Working branch:** `codex/r1d-pvb-d1a1b1-recurring-prop-consumer-lifecycle-hardening` in `C:\Users\guyna\.codex\worktrees\2371\Small_Heroes`. The exhausted live attempt was bound to exact pushed `HEAD` `eeaca686a00ace669d2dda7bd25041683ca650b9` at `0/0` parity. This closeout adds one local documentation-only commit and leaves it unpushed.
...
## R1D-PVB-D1A1B1-PER-ATTEMPT-TYPED-DRAFT-VALIDATION-DIAGNOSTICS-HARDENING - planning gate approved for separate implementation

The exhausted run exposed the next narrow general defect: `TemplateRepairExhaustedError` retains exact per-attempt errors and drafts only in memory, while the attempt receipt persists only a count plus broad regex-derived codes. After safe disposal of raw drafts/responses, the final two invariant identities and structural locations cannot be recovered. Another live attempt without improved evidence would be blind.

- The approved architecture creates closed typed issues at every repairable Visual Contract validator/compiler source across draft-schema, draft-contract, and Action Semantic families, and projects existing typed Source-Evidence-ID failure records into the same evidence trail without changing compact-repair routing.
- Locators contain only closed enums, positive page numbers where applicable, and bounded non-negative structural indices. Authored IDs/values/names/paths/hashes, source phrases, prompt/response/draft material, provider messages, exception prose, stacks, credentials, and arbitrary strings are forbidden.
- Each attempt will persist bounded canonical emitted/current/new/persistent/resolved counts and items, a 128-item cap with `truncated`, and a final-attempt marker. Attempt chronology is preserved. Broad summary codes are derived from typed families, not prose regex.
- Receipt cuts over `v11 -> v12`; readiness cuts over `v9 -> v10` and copies the receipt's ordered diagnostic trail/status without reinterpretation. Request v10, candidate v7, OpenAI evidence v3, provider-failure v2, B0 input/manifest/verification v6/v8/v8, pre-live readiness v7, Execution Request/readiness/result v7/v7/v5, production Blueprint receipt v4, prompt/schema authority, provider/model/tier, budgets, timeout, retries/fallback, `$4.884/$5.00`, candidate semantics, and downstream policy retain their schema labels and behavior. Future binding artifacts must still be newly materialized with new digests.
- Historical artifacts are never migrated, rewritten, redigested, or promoted. Implementation must prove exhaustive producer coverage, transition semantics, privacy, caps, exact-key persistence/reload, current/legacy behavior, unchanged repair inputs/routing and Blueprint contract, and provider/credential/render/storage/downstream unreachability, then receive independent Claude Code review.
- Guy's standing continuous-execution authorization approves a separate local implementation because this gate discovered no model, budget, provider, downstream-policy, product, visual, or creative change. Discovery of any such required change stops for an amended decision. This documentation Task performs no implementation.
- Durable gate: `docs/ai-workflow/R1D_PVB_D1A1B1_PER_ATTEMPT_TYPED_DRAFT_VALIDATION_DIAGNOSTICS_HARDENING_DECISION_GATE.md`.
- **Next action:** after this documentation-only commit, continue in a dedicated implementation Task on this same branch/worktree. Make no provider, readiness, credential, live, render, or downstream call. After local green and independent Claude QA, a later operational attempt requires a new pushed immutable `HEAD`, B0/Fresh Readiness, Execution Request, pricing check, and separate armed sequence.
