...
**Updated:** 2026-07-23
**Maintainer:** Codex
**Working branch:** `codex/r1d-fox-v3-board-mint` in `C:\Users\guyna\.codex\worktrees\0d94\Small_Heroes`. The zero-cost `R1D-BOARD-MINT-LAUNCHER-HARDENING-QA-FIX` milestone started from clean exact head `a1c414bb47a3ff1bde594340c2da9fdd85fed4c3`, three commits ahead of `origin/codex/r1d-fox-v3-board-mint`. This worktree is the sole writer; the correction commits remain local and unpushed. Canonical main, the parent worktree, and every other worktree/branch remain read-only.

## R1D Board Mint Launcher Hardening QA Fix - implemented locally, awaiting independent re-gate

Claude Code independently reviewed the original hardening range `f44661248631a95caa262ff5a15d9b86ab2ffc11..a1c414bb47a3ff1bde594340c2da9fdd85fed4c3` and returned **HOLD**. Its MAJOR finding was an executable private TypeScript entrypoint that bypassed the canonical launcher; its accompanying MINOR findings covered the network sentinel, unmeasured counter claims, Vision/import wording, stale operator guidance, preflight concurrency, strict parsing, and launcher-result precision. Guy's existing zero-cost code/test/local-preflight approval covers this separate correction. The focused code correction is commit `4429238608733307f76ac8ca83dd365b485d88b3`; this is not an independent PASS, mint retry, board approval, or product/visual acceptance.
...
- Claude Code must independently re-gate the original range plus the correction range through the final documentation head. Falsification targets are private-entrypoint bypasses, capability leakage, shim ordering, strict fail-closed parsing, Windows/cross-platform argument behavior, sentinel signature/ordering gaps, hidden external reachability, exact fetch restoration and overlap behavior, whether preflight loads the formerly failing transitive graph, epistemic accuracy of output, callback/write unreachability, launcher result propagation, and dry/mint/approve compatibility.
- No live mint retry is authorized. Even if independent QA passes this correction, any paid Fox v3 mint requires a new explicit Guy RERUN authorization, fresh topology/identity reconciliation, and a separately bounded paid-call budget.
