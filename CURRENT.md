...
**Updated:** 2026-07-23
**Maintainer:** Codex
**Working branch:** `codex/r1d-fox-v3-board-mint` in `C:\Users\guyna\.codex\worktrees\0d94\Small_Heroes`. The zero-cost `R1D-BOARD-MINT-LAUNCHER-HARDENING` milestone started from clean exact head `f44661248631a95caa262ff5a15d9b86ab2ffc11`, one commit ahead of `origin/codex/r1d-fox-v3-board-mint`. This worktree is the sole writer; all resulting commits remain local and unpushed. Canonical main, the parent worktree, and every other worktree/branch remain read-only.

## R1D Board Mint Launcher Hardening - implemented locally, awaiting independent QA

Guy explicitly approved this general, zero-cost corrective milestone after the authorized Fox v3 rerun failed at the real live renderer import before `generateGPTImage`. The implementation is committed at `80b76887fe3c515324c9ff369c60e3d4a52a98d2`. It establishes one public launcher for dry, live mint, approval, and live-import preflight; fixes the broken usage claims; and proves the exact formerly failing live import graph is reachable without touching any external or write boundary. This is not a mint retry, board/product approval, or independent technical PASS.

### Observed defect and general solution

- The old script header and help directed operators to `npx tsx scripts/mint-set-identity-board.ts` and claimed the tool stayed free of `server-only`. The live renderer actually imports `style01-gptimage -> generation-pipeline/companion-character-sheet -> companions.ts -> server-only`. A direct tsx invocation did not preload `scripts/shims/register-server-only.cjs`, so module evaluation failed before `generateGPTImage`.
- The sole public command is now `node scripts/mint-set-identity-board.cjs ...`, also exposed as package script `npm run mint-set-identity-board -- ...`. The CJS launcher uses `process.execPath`, `require.resolve('tsx/cli')`, argument arrays, and absolute internal paths, so it is Windows-safe and cross-platform without `npx` or global/package-resolution ambiguity. It preloads the repository shim in its own process and passes the same shim as a child preload before the exact local tsx CLI evaluates the private TypeScript entrypoint. The old direct `.ts` path fails explicitly with the canonical replacement command.
- `--preflight-live-imports` is a standalone mode and rejects every additional argument. It has no contract, board identity, output path, registry path, or `MintDeps`. It imports the same `loadLiveRendererModules` and `loadLiveStorageModules` functions used by the live renderer/uploader, validates `generateGPTImage`, `resolveStyle01GptModel`, `uploadContentAddressedObjectNoOverwrite`, and the Vision endpoint/model/adapter surface, but never invokes any of them. A temporary throwing fetch sentinel covers the complete import window and is restored on success or failure.
- Existing dry/mint/approve orchestration, set/registry versions and hashes, content-addressed no-overwrite upload, QA fail-closed behavior, approval separation, LOW/MEDIUM cost fence, no-board compatibility, model/quality/size selection, and provider retry semantics are unchanged. No Fox/story/identity/prop literal was added to shared launcher or preflight code.

### Zero-cost runtime and validation evidence

- Focused final validation: **PASS - 2 files / 39 tests** (`mint-tool.spec.ts` 31/31 and `mint-launcher.spec.ts` 8/8). Coverage includes parse/help, preflight exclusivity, unchanged dry/mint/approve behavior, exact live renderer/storage import success, shim-first real subprocess behavior, old-path refusal, launcher exit propagation, no mint/provider/storage/QA/clock callback reachability, fetch restoration on failure, and byte-hash snapshots of the board registry/candidate roots before and after the subprocess.
- The real subprocess smoke test invokes the canonical launcher through `process.execPath` and uses a child-inherited sentinel that throws on fetch, HTTP, HTTPS, TLS, and external TCP. It permits only named-pipe/loopback IPC required by the local tsx runtime. The exact formerly failing transitive renderer graph and the storage dependency graph both loaded; the expected sentinel error never occurred; registry/candidate snapshots remained identical.
- The separately approved local command shape was `NODE_OPTIONS=--require <repo>/lib/set-identity-board/__tests__/fixtures/deny-network.cjs node scripts/mint-set-identity-board.cjs --preflight-live-imports`. The task's PowerShell capture recorded start `2026-07-23T22:46:03.3570817+03:00`, end `2026-07-23T22:46:04.0918079+03:00`, exit `0`, and elapsed `735ms`. Output was `LIVE-IMPORT PREFLIGHT PASS`, renderer exports `generateGPTImage, resolveStyle01GptModel`, storage export `uploadContentAddressedObjectNoOverwrite`, Vision endpoint `https://api.openai.com/v1/chat/completions`, model `gpt-4o`, fetch attempts `0`, and render/upload/Vision/candidate-registry/approval writes all `0`.
- Two preceding PowerShell timing-wrapper setups failed before the launcher started: first because this PowerShell lacks `Join-String`, then because the unescaped Windows preload path could not be resolved by Node. Neither loaded the launcher, internal entrypoint, or preflight graph. The one actual local preflight invocation is the successful run above. Test subprocess preflights are separate, sentinel-confined validation runs.
- `npx --no-install tsc --noEmit`: **PASS**. Focused tests and `git diff --check`: **PASS**. Literal `npm run check` reached TypeScript **PASS**; after raising only the new subprocess tests' full-load timeout, Vitest reproduced exactly the established **6 failures in 5 ignored-output fixture-dependent files**: `child-lexicon-ages-5-8.spec.ts`, `momentum-gate-koko.spec.ts`, `page-entity-qa.spec.ts`, `set-appearance-ref-budget.spec.ts`, and two cases in `story-read-back-validation.spec.ts`. All new launcher/preflight tests passed under the full load. No ignored output fixture was copied, imported, or fabricated.
- The full suite created four ordinary ignored synthetic test files under `outputs/qa-anchors/test_lion__fp__wardrobe` and `outputs/test-fixtures/story-read-back-regression`. Their current-run timestamps and exact paths were verified, then only those four files were removed. The historical ignored Fox dry preview at `outputs/r1d-fox-v3-board-mint/preflight.registry-preview.json` was preserved unchanged.

### Boundaries, limits, rollback, and next gate

- No image render, OpenAI/provider/LLM/Vision request, fetch/reference download, storage/Supabase/database operation, board/candidate/registry write, approval, promotion, deployment, push, flag/threshold change, or production action occurred. No credential file was loaded, and the preflight path did not read provider or storage credentials. The exact Fox v3 registry entry remains absent and `set-identity-board-candidates` remains absent. No independent provider-account or Supabase-log audit was performed; the zero external-call conclusion is supported by the structurally non-invoking preflight path, throwing sentinels, exact output, unchanged artifact snapshots, and reviewed call graph.
- The tracked implementation changes are limited to the canonical launcher, private entrypoint, mint CLI, package-script route, and focused tests/sentinel. There is no schema/data migration. Rollback is a focused revert of the implementation and state commits.
- Claude Code must independently review the immutable range from `f44661248631a95caa262ff5a15d9b86ab2ffc11` through the final documentation head. It should try to falsify shim ordering, Windows/cross-platform argument behavior, hidden network or client construction during imports, whether preflight loads the exact live renderer/storage graph rather than a mock substitute, side-effect unreachability, failure propagation, old direct-path closure, and dry/mint/approve compatibility.
- No live mint retry is authorized. Even if independent QA passes this corrective milestone, any paid Fox v3 mint requires a new explicit Guy RERUN authorization, fresh topology/identity reconciliation, and the separately bounded paid-call budget. No board exists for Guy to inspect or approve.
