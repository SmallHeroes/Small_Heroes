...
**Updated:** 2026-08-05
**Maintainer:** Codex
**Working branch:** `codex/r1d-pvb-d1a1b1-authority-reference-diagnostic-identity` in `C:\Users\guyna\.codex\worktrees\6331\Small_Heroes`. At this planning intake, local `HEAD`, same-name upstream, and origin were exactly `c7a1711374d03e6c02944c8c39f80ceda1911445` at `0/0` parity with a clean worktree and one worktree owner. This Decision Gate is documentation-only and may create a later local commit; it grants no push or implementation authority.

## R1D-PVB-D1A1B1-RECURRING-PROP-CONSUMER-AND-LIFECYCLE-DOMAIN-HARDENING - Decision Gate proposed / waiting for Guy

Repository investigation of the two typed live failures, `recurring_prop_consumer_forbidden` and `recurring_prop_lifecycle_gated`, found one general authoring-domain ambiguity rather than a reason to weaken Set Board safety. Both failures point to the same provider-authored Set Board node (`set_area_node`, authority 0, area 3, node 3, `spatialNodes.propId`, `recurring_prop`). The compiler correctly interprets that slot as a stable fixed-object binding, projects it into every consumer zone, and rejects it when the prop has a reveal gate or is forbidden on any consuming page.

- The final domains are already correctly separated: Set Board fixed objects are stable across all consumers, while page lifecycle uses `propConstraints`; Blueprint v4 owns page placement support, anchors, reveal-safe neutral geometry, and frame feasibility; Wizard qualification consumes the approved package without inventing another lifecycle model.
- The proposed smallest general fix is provider-facing and draft-only: rename ambiguous draft `spatialNodes.propId` to explicit nullable `spatialNodes.stablePropId`, validate it with the existing fail-closed stable-set invariants, and normalize a valid binding into the unchanged final Set Board representation.
- A stable-set consumer is allowed only for a uniquely declared prop with no `firstRevealPage`, no forbidden consumer page, and one genuinely stable placement. A lifecycle-gated or portable prop never becomes fixed architecture merely because it was revealed.
- A page-frame consumer is allowed only at/after reveal on a page that explicitly marks the prop `required`, with valid Blueprint placement support and exact anchor enforcement when applicable. Before reveal, when forbidden, when merely permitted, or without support, it remains unbound/gated. Neutral support geometry may exist pre-reveal but cannot bind or name the hidden prop.
- Final VC v4, Set Board v4, Blueprint v4, Visual Package v5, runtime v6, Wizard, renderer, model/tier, 64K ceiling, call/repair budget, timeout, retries, fallback, and `$4.884/$5.00` ceilings stay unchanged. The changed draft/prompt and lifecycle-binding authorities cut over fail-closed; historical artifacts remain immutable and cannot authorize a future attempt.
- Acceptance requires exhaustive stable/gated/page-frame tests, Structured Outputs compatibility, compiler/final validation, Blueprint feasibility, zero-cost Wizard qualification with provider/image sentinels unreachable, deterministic TypeScript, `git diff --check`, the policy repository gate, and independent Claude Code QA. The six known ignored-fixture failures remain a separate release HOLD.
- The exact later route remains: approved implementation and QA, pushed immutable head, new Fresh Readiness, one separately authorized bounded live authoring attempt, valid candidate, actual Semantic Reconciliation, Blueprint/Visual Package and Wizard qualification, then a separately priced and approved single local `gpt-image-2` LOW portrait render. No full-book render or downstream production action is authorized here.
- Durable planning record: `docs/ai-workflow/R1D_PVB_D1A1B1_RECURRING_PROP_CONSUMER_AND_LIFECYCLE_DOMAIN_HARDENING_DECISION_GATE.md`.
- **Next action:** Guy must approve, amend, or reject the nine architectural decisions. No production-code implementation, Fresh Readiness, provider call, candidate, Blueprint/Wizard execution, or render is authorized by this planning record.
