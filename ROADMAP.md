...
- **R3-B1b duplicate coverage-cardinality recovery Decision Gate:** Guy authorized zero-cost Gate preparation only. Investigation found that the compact page-repair admission can offer a one-field action-binding target when the complete page graph has no legal uncovered action beat; the captured P1 graph therefore made duplication inevitable. The prepared general design adds graph-aware closed admission, atomic post-patch cardinality proof, bounded escalation of complete pure action/coverage-binding failures to the existing `full_draft` lane, and a versioned cutover that preserves the reviewed v55/v58/v55 replay exactly. Claude Code independently reviewed `11d4ff0a..3924ad62` and returned PASS with no P0/P1 and one P2: the original “cleanup eligibility unchanged” claim ignored that `full_draft`, unlike `page_contract_patch`, is an eligible predecessor. The corrected Gate truthfully records the flip and requires a new route-specific exclusion so cardinality-escalated full drafts cannot unlock cleanup while ordinary existing cleanup behavior stays unchanged. Claude Code independently re-gated exact correction range `3924ad62..5cb3e813` and returned PASS with no P0/P1/P2, closing the P2. No code, provider call, retry or downstream work is authorized.
