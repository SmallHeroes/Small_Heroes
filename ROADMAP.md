...
**Last verified:** 2026-09-06 (P1 execution QA closeout and semantic-recovery plan)
...
- **Next gate prepared:** `docs/ai-workflow/R3B1B_GENERAL_SEMANTIC_RECOVERY_DECISION_GATE.md` proposes general accepted supporting-cast/appearance/group authority, fast-locomotion fidelity, atomic candidate recovery and effective-template/coverage bridge validation, with zero provider/image spend. Guy's cover and page-12 cart dispositions are still pending; implementation has not started. Reuse the paid candidate first; re-authoring is a separately gated fallback only if recovery fails. No story-specific code. Blueprint, Board/prop images, package work, promotion, rendering, P2, narration, publication and deployment remain unauthorized. P6 Cowork, HOLD/D materialization and the soft narration items remain separate gates.
