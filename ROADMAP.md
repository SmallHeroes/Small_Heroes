...
- **Next gate:** prepare the six digest-bound technical-review envelopes as one bounded zero-cost batch, then independently verify that envelope set before returning publication/package sequencing and the six exact future revision digests to Guy. Preparation keeps sellability at 17/18; publishing all six before exact-source packages exist is expected to reduce it to 11/18 and is not authorized by the P1-only 17/18 decision. P1 package/Visual Contract work remains a separate decision and spend gate. P6 still goes to Cowork before any later Guy acceptance; D decisions must be materialized into new candidate digests before HOLD records can enter acceptance. The 10 soft items require separate narration-pronunciation and human-ear work and cannot be inferred away from automated preflight. The current approval grants no publication, package, render or paid work.
