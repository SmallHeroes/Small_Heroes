...
describe('canonical live authoring executable boundary', () => {
  ...
  it('routes a shape-valid immediate-predecessor request only through rejected evidence and current failed lifecycle envelopes', async () => {
    ...
    request.version = 'visual-contract-authoring-request/v37';
    redigestRequest(request);
    writeJson(
      fixture.repoRoot,
      fixture.requestPath,
  ...
  it.each([0, 1, 2, 3])(
    'completes after %i semantic repair(s) within the four-call fence',
    async (repairCount) => {
      const fixture = createLiveFixture(
        `repairs-${repairCount}`,
      );
      ...
      expect(result.receipt.attempts[0]
        ?.reservedExposureBeforeCallUsd).toBe(6.6275);
  ...
  it('fails after exactly three semantic repairs and persists no candidate', async () => {
