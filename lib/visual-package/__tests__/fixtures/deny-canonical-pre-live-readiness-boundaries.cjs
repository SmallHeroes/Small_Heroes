...
const PREFIX =
  'TEST_CANONICAL_PRE_LIVE_READINESS_SENTINEL';
const credentialPath =
  process.env.CANONICAL_PRE_LIVE_DENIED_CREDENTIAL_PATH;
const allowedOutputRoot =
  process.env.CANONICAL_PRE_LIVE_ALLOWED_OUTPUT_ROOT;
...
fs.openSync = function boundedOpen(value, flags) {
  ...
  const descriptor = originals.openSync.apply(
    this,
    arguments,
  );
...
childProcess.spawn = function denySpawn() {
  throw new Error(`${PREFIX}:live_child`);
};
childProcess.spawnSync = function allowOnlyGit(executable) {
  ...
  return originals.spawnSync.apply(this, arguments);
};
...
if (control === 'credential_exists') {
  fs.existsSync(credentialPath);
}
if (control === 'credential_stat') {
  fs.statSync(credentialPath);
}
if (control === 'credential_lstat') {
  fs.lstatSync(credentialPath);
}
if (control === 'credential_realpath') {
  fs.realpathSync(credentialPath);
}
if (control === 'credential_open') {
  fs.openSync(credentialPath, 'r');
}
if (control === 'credential_read') {
...
if (control === 'live_child') {
  childProcess.spawn(process.execPath, ['-e', 'void 0']);
}
if (control === 'write') {
  fs.writeFileSync(
    path.join(
      path.dirname(allowedOutputRoot),
      'forbidden-write',
    ),
    'x',
  );
