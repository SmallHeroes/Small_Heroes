...
function exerciseCacheRootSignature() {
  fs.mkdirSync(cacheRoot, { recursive: true });
}

async function exerciseCacheFileAlias() {
  await fs.promises.writeFile(cacheFile, 'mutated');
}
...
async function main() {
  ...
  if (mode === 'cache-root-signature') {
    return exerciseCacheRootSignature();
  }
  if (mode === 'cache-file-alias') {
    return exerciseCacheFileAlias();
  }
