...
import {
  ...
  visualContractAuthoringReceiptPromptAuthorityIssues,
        ...
        : [
            {
              ...
              disposition: {
                ...
                representedValue: page.locationId,
              },
            },
          ];
  }
...
function withPageEightRepresentedElsewhereResidual(
  base: BookVisualContractTemplate & Record<string, unknown>,
  permittedValue: string,
): BookVisualContractTemplate & Record<string, unknown> {
  const next = structuredClone(base);
  const representedPage = next.pageContracts[7] as unknown as {
    pageNumber: number;
    propState: Array<{ propId: string; state: string }>;
  ...
  const representedSource = representedPage.actionSemanticCoverage[0];
  if (!representedSource) {
    throw new Error('Expected seeded page-eight Source Evidence');
  }
  if (!representedPage.propState[0]) {
    throw new Error('Expected seeded page-eight prop state');
  }
  representedPage.propState[0].state = permittedValue;
  representedPage.actionSemanticCoverage = [
    ...representedPage.actionSemanticCoverage,
    {
      beatId: 'beat:p8:post_cleanup_represented_residual',
      sourceEvidenceId: representedSource.sourceEvidenceId,
      disposition: {
        kind: 'represented_elsewhere',
        contractPointer: '/pageContracts/99/propState/0/state',
        contractValue: 'stale represented value',
      },
    },
  ];
  return next;
...
function invalidMaterialDraftSequence(
  snapshot: StorySourceAuthoritySnapshot,
  count: number,
): readonly (BookVisualContractTemplate & Record<string, unknown>)[] {
  return Array.from({ length: count }, (_value, index) => {
    const draft = fullyActionedBunnyDraft(snapshot);
    draft.recurringProps[0]!.material = '';
    draft.recurringProps[0]!.description =
      `${draft.recurringProps[0]!.description} Attempt ${index + 1}.`;
    return draft;
  });
}
...
function liveShapedAtomicBindingFixture(
  ...
  seedDuplicateComponents(6, ['beat:p6:live_component_a']);
  seedDuplicateComponents(7, ['beat:p7:live_component_a']);
  seedDuplicateComponents(11, [
    'beat:p11:live_component_a',
    'beat:p11:live_component_b',
  ]);
  seedDuplicateComponents(12, ['beat:p12:live_component_a']);
  ...
  page11.actionSemanticCoverage.push(
    {
  ...
  const normalized = normalizeExactActionBindingComponents({
    pages: initial.pageContracts as unknown as Record<string, unknown>[],
    sourceEvidenceCatalog: snapshot.content.sourceEvidenceCatalog,
  });
  if (normalized.normalizations.length !== 6) {
    throw new Error('expected six live-shaped compiler normalizations');
  }
  const afterAtomicPageRepair = {
    ...structuredClone(initial),
    pageContracts: normalized.pages as unknown as typeof initial.pageContracts,
  };
...
const BOOK_SURFACE_STRUCTURAL_PATCH_FIELDS = [
  'pageNumber',
  'locationId',
  'zoneId',
  'sameLocationAs',
...
function bookSurfaceV7Response(args: {
  ...
  if (!presentationTargets || !affectedPages) {
    throw new Error('missing book-surface v7 fixture authority');
  }
  ...
  return {
    ...
    pageStructuralPatches: affectedPages.map((affectedPage) => {
      ...
      return Object.fromEntries(
        BOOK_SURFACE_STRUCTURAL_PATCH_FIELDS.map((field) => [
          field,
          field === 'pageNumber' || writableFields.has(field)
            ? structuredClone(repairedPage[field])
            : null,
        ]),
      );
    }),
...
describe('Story Source authority snapshot', () => {
  ...
  it('carries a neutral reusable source as explicit gender-flexible authoring authority', () => {
    ...
    expect(snapshot.content.sourceGenderMode).toBe('neutral');
    expect(input.childGender).toBe(
      'gender-flexible source (boy/girl resolved at runtime)',
    );
    expect(prompt).toContain(
      'child: (child) (gender-flexible source (boy/girl resolved at runtime))',
    );
    expect(prompt).not.toContain('child: (child) (female)');

    const tampered = structuredClone(snapshot);
    (tampered.content as unknown as { sourceGenderMode: string }).sourceGenderMode =
      'arbitrary';
    expect(() =>
      assertValidStorySourceAuthoritySnapshot(tampered),
    ).toThrow('snapshot source gender mode is stale or malformed');

    const invalid = writeStoryFixture({ gender: 'arbitrary' });
    expect(() => snapshotFor(invalid)).toThrow(
      'Story Source gender mode is invalid',
    );
  });
...
describe('exact zero-cost authoring preflight', () => {
  ...
  it('locks every approved request surface and keeps the injected provider unreachable', async () => {
    ...
    expect(request).toMatchObject({
      ...
      policyVersion: 'visual-contract-authoring-policy/v22',
      ...
      promptAuthority: {
        ...
        pageContractRepair: {
          systemPromptVersion: 'page-contract-repair-prompt/v13',
          userPromptVersion:
  ...
  it('rejects a forged request_invalid zero-attempt receipt that launders the invalid request schedule', async () => {
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/missing-rejected-request-budget',
        request,
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/rejected-request-budget',
        request,
        receipt: rejected.receipt,
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/forged-rejected-request-budget',
        request,
    ...
    const predecessorRequest = structuredClone(request);
    predecessorRequest.version =
      'visual-contract-authoring-request/v33' as typeof predecessorRequest.version;
    const {
      digestAlgorithm: _predecessorRequestDigestAlgorithm,
      digest: _predecessorRequestDigest,
      ...predecessorRequestPayload
    } = predecessorRequest;
    predecessorRequest.digest = canonicalJsonDigest(
      predecessorRequestPayload,
    );
    const predecessorReceipt = structuredClone(rejected.receipt);
    predecessorReceipt.requestDigest = predecessorRequest.digest;
    const {
      digestAlgorithm: _predecessorReceiptDigestAlgorithm,
      digest: _predecessorReceiptDigest,
      ...predecessorReceiptPayload
    } = predecessorReceipt;
    predecessorReceipt.digest = canonicalJsonDigest(
      predecessorReceiptPayload,
    );
    const predecessorReadiness = structuredClone(validReadiness);
    predecessorReadiness.authoringRequestDigest =
      predecessorRequest.digest;
    predecessorReadiness.authoringReceiptDigest =
      predecessorReceipt.digest;
    const {
      digestAlgorithm: _predecessorReadinessDigestAlgorithm,
      digest: _predecessorReadinessDigest,
      ...predecessorReadinessPayload
    } = predecessorReadiness;
    predecessorReadiness.digest = canonicalJsonDigest(
      predecessorReadinessPayload,
    );
    expect(() =>
      persistVisualContractAuthoringReadiness({
        repoRoot: tempRoot(),
        outputDir: 'outputs/predecessor-request-rejected-readiness',
        request: predecessorRequest,
        evidence: predecessorReadiness,
    ...
    const {
      digestAlgorithm: _readinessDigestAlgorithm,
      digest: _readinessDigest,
      ...readinessPayload
    } = forgedReadiness;
    forgedReadiness.digest = canonicalJsonDigest(readinessPayload);
    expect(() =>
      persistVisualContractAuthoringReadiness({
        repoRoot: tempRoot(),
        outputDir: 'outputs/forged-rejected-request-readiness',
        request,
  ...
  it.each([
    [
      ...
      (request: ReturnType<typeof requestFor>) => {
        request.representedElsewhereRepairStructuredOutput.schemaDigest =
          'd'.repeat(64);
      },
    ],
    [
      ...
      'prompt_authority_mismatch',
      (request: ReturnType<typeof requestFor>) => {
        request.promptAuthority.representedElsewhereRepair.systemPromptDigest =
          'c'.repeat(64);
      },
    ],
  ])('rejects altered represented-elsewhere repair %s before provider reachability', async (
    _label,
    expectedIssue,
    mutate,
  ) => {
  ...
  it.each([
    ...
    [
      ...
      'prompt_authority_mismatch',
      (request: ReturnType<typeof requestFor>) => {
        request.promptAuthority.bookSurfaceRepair.systemPromptDigest =
          '8'.repeat(64);
      },
    ],
  ])('rejects altered book-surface repair %s before provider reachability', async (
    _label,
    expectedIssue,
    mutate,
  ) => {
  ...
  it.each([
    [
      ...
      (request: ReturnType<typeof requestFor>) => {
        request.stablePropScopeRepairStructuredOutput.schemaDigest =
          '7'.repeat(64);
      },
    ],
    [
      ...
      'prompt_authority_mismatch',
      (request: ReturnType<typeof requestFor>) => {
        request.promptAuthority.stablePropScopeRepair.systemPromptDigest =
          '6'.repeat(64);
      },
    ],
  ])('rejects altered stable-prop scope repair %s before provider reachability', async (
    _label,
    expectedIssue,
    mutate,
  ) => {
  ...
  it.each([17, 23])(
    ...
    expect(() =>
      ...
      /readiness v56 requires exact typed draft-validation evidence/,
    );
  });
  ...
  it('reserves remaining multi-call exposure conservatively before each provider call', () => {
    ...
    expect(
      authoringSpendIsWithinCeiling(
        authoringReservedExposureUsd({
          request,
          conservativeAccountedCostUsd: 1.7,
          providerCallsCompleted: 1,
        }),
        request.costBudget.hardCeilingUsd,
      ),
  ...
  it('rejects the prior policy and July pricing after re-digesting a current outer request', async () => {
    ...
    expect(request.version).toBe('visual-contract-authoring-request/v56');
...
describe('source-grounded closed action authority', () => {
  ...
  it('rejects an action-bound coverage record when its same-page action list is empty', async () => {
    ...
    expect(thrown).toBeInstanceOf(DraftAuthorityReferenceDomainError);
    expect(
      (thrown as DraftAuthorityReferenceDomainError).issues,
    ).toEqual([
      {
  ...
  it('rejects minted and malformed action authority but normalizes an exact duplicate binding without removing action authority', async () => {
    const snapshot = bunnySnapshot();
    ...
    for (const page of malformed.pageContracts) {
      const draftPage = page as unknown as Record<string, unknown>;
      draftPage.propConstraints ??= [];
      draftPage.actionRequirements ??= [];
    }
    (
      (
    ...
    const duplicateProvider = vi.fn(async () => JSON.stringify(duplicate));
    const duplicateResult = await compileBookVisualContractTemplate(
      {
        ...snapshot.content,
        storyKey: snapshot.content.storyKey,
        pageCount: snapshot.content.pages.length,
      ...
      { callLLM: duplicateProvider },
    );
    expect(duplicateProvider).toHaveBeenCalledTimes(1);
    expect(duplicateResult.repairAttempts).toEqual([]);
    expect(
      duplicateResult.template.pageContracts[0]!.actionRequirements,
    ).toHaveLength(2);
    expect(
      new Set(
        duplicateResult.template.pageContracts[0]!.actionRequirements!.map(
...
describe('sanitized receipts and immutable artifact lifecycle', () => {
  ...
  it('classifies every prior authoring authority as immutable without mutating its bytes', () => {
    const immediatelyPrior = [
      ['request', 'visual-contract-authoring-request/v51'],
      ['receipt', 'visual-contract-authoring-receipt/v53'],
      ['readiness', 'visual-contract-authoring-readiness/v51'],
      ['candidate', 'visual-contract-candidate-artifact/v8'],
    ] as const;
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v18',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v25',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v31',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v32',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v33',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v34',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v35',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v38',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v39',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v42',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v43',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v44',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v51',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v56',
      ),
    ).toBe('current');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v49',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'request',
        'visual-contract-authoring-request/v48',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'receipt',
        'visual-contract-authoring-receipt/v28',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'receipt',
        'visual-contract-authoring-receipt/v36',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'receipt',
        'visual-contract-authoring-receipt/v37',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'receipt',
        'visual-contract-authoring-receipt/v42',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'receipt',
        'visual-contract-authoring-receipt/v48',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'receipt',
        'visual-contract-authoring-receipt/v49',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'receipt',
        'visual-contract-authoring-receipt/v50',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'receipt',
        'visual-contract-authoring-receipt/v51',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v19',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v26',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v32',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v33',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v34',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v35',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v36',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v37',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v40',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v41',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v44',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v45',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v46',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v47',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v48',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v49',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v51',
      ),
    ).toBe('legacy_immutable');
    expect(
      visualContractAuthoringArtifactVersionStatus(
    ...
    expect(
      visualContractAuthoringArtifactVersionStatus(
        'readiness',
        'visual-contract-authoring-readiness/v56',
      ),
    ).toBe('current');
    expect(
      visualContractAuthoringArtifactVersionStatus(
  ...
  it('rejects the immediately prior receipt as current readiness authority after canonical redigest', async () => {
    ...
    prior.version = 'visual-contract-authoring-receipt/v54';
    const {
      digestAlgorithm: _digestAlgorithm,
      digest: _digest,
      ...payload
  ...
  it('rejects the immediately prior readiness as current authority after canonical redigest', async () => {
    const snapshot = bunnySnapshot();
    const request = requestFor(snapshot, 'live');
    const result = await runVisualContractAuthoring({
      request,
    ...
    const prior = structuredClone(readiness) as unknown as Record<
      string,
      unknown
    >;
    prior.version = 'visual-contract-authoring-readiness/v51';
    const {
      digestAlgorithm: _digestAlgorithm,
      digest: _digest,
      ...payload
    ...
    expect(() =>
      persistVisualContractAuthoringReadiness({
        repoRoot: tempRoot(),
        outputDir: 'outputs/immediate-readiness-predecessor',
        request,
        evidence: prior as unknown as typeof readiness,
        receipt: result.receipt,
        write: false,
      }),
  ...
  it('rejects persistence of re-digested exhaustion receipt evidence bound to only one logical call and zero repairs', async () => {
    ...
    const result = await runVisualContractAuthoring({
      request,
      snapshot,
      provider: sequencedSuccessfulProvider(
        invalidMaterialDraftSequence(snapshot, 7),
      ),
    });
    expect(result.receipt.failure).toMatchObject({
    ...
    expect(() =>
      ...
      /receipt v59 requires exact typed draft-validation evidence/,
    );
  });
  ...
  it('sanitizes provider failures and refuses provider/model substitution', async () => {
    ...
    expect(mismatch.receipt.attempts[0]?.policyMismatchScope).toBe(
      'provider_identity',
    );
    expect(
      visualContractAuthoringReceiptPromptAuthorityIssues({
    ...
    const falseProviderIdentityClaim = structuredClone(
      mismatch.receipt,
    );
    falseProviderIdentityClaim.attempts[0]!.provider =
      request.provider;
    falseProviderIdentityClaim.attempts[0]!.model = request.model;
    expect(
      visualContractAuthoringReceiptPromptAuthorityIssues({
        request,
        receipt: falseProviderIdentityClaim,
      }),
    ).toContain('receipt_attempt_policy_mismatch_scope_invalid');

    const callOptionsClaim = structuredClone(mismatch.receipt);
    const callOptionsAttempt = callOptionsClaim.attempts[0]!;
    callOptionsAttempt.policyMismatchScope = 'call_options';
    callOptionsAttempt.providerReached = false;
    callOptionsAttempt.provider = request.provider;
    callOptionsAttempt.model = request.model;
    callOptionsAttempt.systemPromptDigest = '4'.repeat(64);
    callOptionsAttempt.validationDiagnostics = {
      count: 1,
      codes: ['provider_policy_mismatch'],
    };
    expect(
      visualContractAuthoringReceiptPromptAuthorityIssues({
        request,
        receipt: callOptionsClaim,
      }),
    ).toContain('receipt_attempt_static_prompt_authority_mismatch');

    const falseBudgetSequenceClaim = structuredClone(
      callOptionsClaim,
    );
    falseBudgetSequenceClaim.attempts[0]!.policyMismatchScope =
      'budget_sequence' as never;
    falseBudgetSequenceClaim.attempts[0]!.systemPromptDigest =
      mismatch.receipt.attempts[0]!.systemPromptDigest;
    expect(
      visualContractAuthoringReceiptPromptAuthorityIssues({
        request,
        receipt: falseBudgetSequenceClaim,
      }),
    ).toContain('receipt_attempt_policy_mismatch_scope_invalid');

    const promptAuthorityClaim = structuredClone(callOptionsClaim);
    promptAuthorityClaim.attempts[0]!.policyMismatchScope =
      'prompt_authority';
    expect(
      visualContractAuthoringReceiptPromptAuthorityIssues({
        request,
        receipt: promptAuthorityClaim,
      }),
    ).toEqual([]);
    promptAuthorityClaim.attempts[0]!.repairMode =
      'unknown_repair_mode' as never;
    promptAuthorityClaim.attempts[0]!.kind = 'repair';
    expect(
      visualContractAuthoringReceiptPromptAuthorityIssues({
        request,
        receipt: promptAuthorityClaim,
      }),
    ).toContain('receipt_attempt_prompt_route_invalid');
  });
  ...
  it('repairs page-spatial authority first when canonical topology identity is still invalid', async () => {
    const snapshot = bunnySnapshot();
    const invalid = fullyActionedBunnyDraft(snapshot);
  ...
  it('persists exact cardinality-escalation provenance and rejects missing, forged, or unrelated frontiers', async () => {
    const snapshot = storySnapshot('fox_uri_adventure');
    const request = requestFor(snapshot, 'live');
    const valid = actionCapabilityCalibrationDraft(snapshot);
    const invalid = structuredClone(valid);
    const targetPage = invalid.pageContracts
      .map(
        (page) =>
          page as unknown as {
            pageNumber: number;
      ...
      .find(
        (page) =>
          (page.actionRequirements?.length ?? 0) > 0 &&
          (page.actionSemanticCoverage?.length ?? 0) > 0,
      );
    if (!targetPage?.actionSemanticCoverage) {
      throw new Error('missing action calibration page');
    }
    targetPage.actionSemanticCoverage.push(
      structuredClone(targetPage.actionSemanticCoverage[0]!),
    );
    const provider: VisualContractAuthoringProvider = {
      call: vi.fn(async (call) => ({
        output: JSON.stringify(call.attempt === 1 ? invalid : valid),
        receipt: {
          provider: 'openai',
          model: 'gpt-5.6-sol',
          responseId: `cardinality-escalation-${call.attempt}`,
          usage: {
            input_tokens: 1_000,
            output_tokens: 2_000,
            total_tokens: 3_000,
    ...
    expect(
      result.receipt.attempts.map((attempt) => ({
        repairMode: attempt.repairMode,
        routeProvenance: attempt.routeProvenance,
      })),
    ).toEqual([
      { repairMode: null, routeProvenance: null },
      {
        repairMode: 'full_draft',
        routeProvenance:
          VISUAL_CONTRACT_AUTHORING_CARDINALITY_ESCALATION_PROVENANCE,
    ...
    const activeFrontier = result.receipt.attempts[0]!
      .draftValidationDiagnostics!.items.filter(
        (item) => item.state !== 'resolved',
      )
      .map((item) => item.issue);
    expect(activeFrontier).toEqual(
      expect.arrayContaining([
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        ...
        outputDir: 'outputs/.cardinality-provenance-validation',
        request,
        receipt: result.receipt,
        write: false,
      }),
    ...
    const redigest = (receipt: typeof result.receipt) => {
      const {
        digestAlgorithm: _digestAlgorithm,
        digest: _digest,
        ...payload
    ...
    const expectRejected = (receipt: typeof result.receipt) => {
      expect(
        visualContractAuthoringAttemptBudgetSequenceIsValid(
          receipt.attempts,
    ...
    missing.attempts[1]!.routeProvenance = null;
    expectRejected(missing);

    const forged = structuredClone(result.receipt);
    forged.attempts[0]!.routeProvenance =
      VISUAL_CONTRACT_AUTHORING_CARDINALITY_ESCALATION_PROVENANCE;
    expectRejected(forged);

    const unrelated = structuredClone(result.receipt);
    const unrelatedIssue = {
      family: 'action_semantic' as const,
      code: 'coverage_missing' as const,
      locator: {
        kind: 'page' as const,
        fieldRole: 'coverage' as const,
        pageNumber: targetPage.pageNumber,
      },
    };
    const unrelatedFrontier = activeFrontier.map((issue) =>
      issue.family === 'action_semantic' &&
      issue.code === 'beat_identity_duplicate'
        ? unrelatedIssue
        : issue,
    );
    const unrelatedTrail = buildDraftValidationDiagnosticTrail([
      unrelatedFrontier,
      [],
    ]);
    unrelated.attempts[0]!.draftValidationDiagnostics = structuredClone(
      unrelatedTrail[0]!,
    );
    unrelated.attempts[0]!.validationDiagnostics = {
      count: unrelatedFrontier.length,
      codes: draftValidationDiagnosticCodesForIssues(unrelatedFrontier),
    };
    unrelated.attempts[1]!.draftValidationDiagnostics = structuredClone(
      unrelatedTrail[1]!,
    );
    unrelated.attempts[1]!.validationDiagnostics = {
      count: 0,
      codes: draftValidationDiagnosticCodesForIssues([]),
    };
  ...
  it('persists an eighth-call terminal reference cleanup and rejects budget-sequence tampering', async () => {
    ...
    const cleanupPages = referenceOnlyResidual.pageContracts.slice(0, 3);
    expect(cleanupPages).toHaveLength(3);
    for (const referencePage of cleanupPages) {
      ...
      const cleanupId = `fixture_cleanup_chair_p${referencePage.pageNumber}`;
      referenceZone.spatialNodes = [
        ...(referenceZone.spatialNodes ?? []),
        {
          id: cleanupId,
          kind: 'furniture',
          description: `Stable cleanup target chair for page ${referencePage.pageNumber}.`,
        },
      ];
      ...
      action.object = {
        kind: 'spatial',
        id: `outside_current_page_${referencePage.pageNumber}_action_zone`,
      };
    ...
    for (const page of initialSpatialAndLatentFailure.pageContracts.slice(0, 3)) {
      page.actionRequirements![0]!.object = {
        kind: 'spatial',
        id: `outside_initial_page_${page.pageNumber}_action_zone`,
      };
    }
    for (const page of initialSpatialAndLatentFailure.pageContracts.slice(3, 6)) {
      page.camera = '';
    }
    const worldOnlyResidual = fullyActionedBunnyDraft(snapshot);
    worldOnlyResidual.worldType = '';
    for (const page of worldOnlyResidual.pageContracts.slice(3, 6)) {
      page.camera = '';
    }
    const provider: VisualContractAuthoringProvider = {
      call: vi.fn(async (call) => {
        ...
        if (call.attempt === 1) {
          ...
          variant.recurringProps[0].description =
            `${variant.recurringProps[0].description} Attempt ${call.attempt}.`;
          output = variant;
          ...
          output = {
            patches: payload.targets.map((target) => ({
              ...
              spatialReferenceId:
                target.permittedSpatialReferences.find(
                  (reference) =>
                    reference.id ===
                    `fixture_cleanup_chair_p${target.pageNumber}`,
                )?.id ?? target.permittedSpatialReferences[0]!.id,
            })),
          };
        }
        return {
    ...
    const initialWithRepresentedResidual =
      withPageEightRepresentedElsewhereResidual(
        initialSpatialAndLatentFailure,
        'Represented authority target',
      );
    const worldWithRepresentedResidual =
      withPageEightRepresentedElsewhereResidual(
        worldOnlyResidual,
        'Represented authority target',
      );
    const representedAfterCleanupResidual =
      withPageEightRepresentedElsewhereResidual(
        referenceOnlyResidual,
        `Oversized represented authority ${'R'.repeat(70_000)}`,
      );
    const providerForTerminalResidual = (
      terminalResidual: BookVisualContractTemplate & Record<string, unknown>,
    ): VisualContractAuthoringProvider => ({
      call: vi.fn(async (call) => {
        let output: unknown;
        if (call.attempt === 1) {
          output = initialWithRepresentedResidual;
        } else if (
          call.attempt === 3 ||
          call.attempt === 4 ||
          call.attempt === 5 ||
          ...
          const variant = structuredClone(worldWithRepresentedResidual);
          variant.recurringProps[0].description =
            `${variant.recurringProps[0].description} Post-cleanup attempt ${call.attempt}.`;
          output = variant;
        } else if (call.attempt === 7) {
          output = terminalResidual;
        } else {
          const payload = JSON.parse(call.userPrompt) as {
            targets: Array<{
              pageNumber: number;
        ...
        return {
          ...
          receipt: {
            provider: 'openai',
            model: 'gpt-5.6-sol',
            responseId: `post-cleanup-response-${call.attempt}`,
            usage: {
              input_tokens: 1_000,
              output_tokens: 1_000,
              total_tokens: 2_000,
    ...
    const postCleanupRepresentedProvider =
      providerForTerminalResidual(representedAfterCleanupResidual);
    const postCleanupRepresentedResult = await runVisualContractAuthoring({
      request,
      snapshot,
      provider: postCleanupRepresentedProvider,
    });
    expect(postCleanupRepresentedProvider.call).toHaveBeenCalledTimes(7);
    expect(postCleanupRepresentedResult.receipt).toMatchObject({
      status: 'failed',
      callCount: 7,
      repairCount: 6,
    ...
    expect(
      postCleanupRepresentedResult.receipt.attempts[6]
        ?.draftValidationDiagnostics,
    ).toMatchObject({
      currentUniqueCount: 7,
      finalAttempt: true,
    });
    expect(() =>
      buildVisualContractAuthoringReadinessEvidence({
        snapshot,
        request,
        receipt: postCleanupRepresentedResult.receipt,
      }),
    ).not.toThrow();

    const pureRepresentedAtStandardExhaustion =
      withPageEightRepresentedElsewhereResidual(
        fullyActionedBunnyDraft(snapshot),
        `Oversized represented prop-state authority ${'P'.repeat(70_000)}`,
      );
    const pureRepresentedProvider = providerForTerminalResidual(
      pureRepresentedAtStandardExhaustion,
    );
    const pureRepresentedResult = await runVisualContractAuthoring({
      request,
      snapshot,
      provider: pureRepresentedProvider,
    });
    expect(pureRepresentedProvider.call).toHaveBeenCalledTimes(7);
    expect(pureRepresentedResult.receipt).toMatchObject({
      status: 'failed',
      callCount: 7,
      repairCount: 6,
      draftValidationStatus: 'repair_exhausted',
    ...
    const pureFinalDiagnostics =
      pureRepresentedResult.receipt.attempts[6]
        ?.draftValidationDiagnostics;
    expect(pureFinalDiagnostics).toMatchObject({
      currentUniqueCount: 1,
      finalAttempt: true,
    });
    const pureRepresentedAuthority = representedElsewhereRepairAuthority({
      draft: pureRepresentedAtStandardExhaustion,
      diagnosticIssues:
        pureFinalDiagnostics?.items
          .filter((item) => item.state !== 'resolved')
          .map((item) => item.issue) ?? [],
      pointerTemplate:
        pureRepresentedAtStandardExhaustion as unknown as
          ActionSemanticCoverageTemplate,
      sourceEvidenceCatalog:
        snapshot.content.sourceEvidenceCatalog,
    });
    expect(pureRepresentedAuthority).not.toBeNull();
    const pureRepresentedInputAccounting =
      visualContractAuthoringInputAccounting(
        buildRepresentedElsewhereRepairSystemPrompt(),
        buildRepresentedElsewhereRepairUserPrompt({
          authority: pureRepresentedAuthority!,
        }),
        REPRESENTED_ELSEWHERE_REPAIR_JSON_SCHEMA,
      );
    ...
    for (const zone of oversizedReferenceOnlyResidual.zones) {
      if (!zone.spatialNodes) {
        continue;
      }
      zone.spatialNodes = zone.spatialNodes.map((node) => ({
        ...node,
        description: `${node.description}${'x'.repeat(700)}`,
      }));
      zone.stableGeometry = projectZoneStableGeometry(zone)!;
    }
    const oversizedProvider: VisualContractAuthoringProvider = {
      call: vi.fn(async (call) => {
        let output: unknown;
        if (call.attempt === 1) {
          output = initialSpatialAndLatentFailure;
          ...
          const variant = structuredClone(worldOnlyResidual);
          variant.recurringProps[0].description =
            `${variant.recurringProps[0].description} Oversized attempt ${call.attempt}.`;
          output = variant;
        } else if (call.attempt === 7) {
        ...
        return {
          ...
          receipt: {
            provider: 'openai',
            model: 'gpt-5.6-sol',
            responseId: `oversized-cleanup-response-${call.attempt}`,
            usage: {
              input_tokens: 1_000,
              output_tokens: 1_000,
              total_tokens: 2_000,
    ...
    const oversizedResult = await runVisualContractAuthoring({
      request,
      snapshot,
    ...
    for (const tamper of [
      ...
      (receipt: typeof result.receipt) => {
        receipt.attempts[6]!.routeProvenance =
          VISUAL_CONTRACT_AUTHORING_CARDINALITY_ESCALATION_PROVENANCE;
      },
      (receipt: typeof result.receipt) => {
      ...
      (receipt: typeof result.receipt) => {
        const current = receipt.attempts[6]!
          .draftValidationDiagnostics!.items.find(
            (item) =>
              item.state !== 'resolved' &&
              item.issue.locator.kind === 'page_item',
          );
      ...
      expect(
        visualContractAuthoringAttemptBudgetSequenceIsValid(
          forged.attempts,
          forged.standardAttemptOutputBudget,
        ),
      ).toBe(false);
      const {
        digestAlgorithm: _digestAlgorithm,
    ...
    const forgedReadiness = structuredClone(readiness);
    const forgedReadinessLimits =
      forgedReadiness.standardAttemptOutputBudget
        .limits as unknown as number[];
    forgedReadinessLimits[0] = forgedReadinessLimits[0]! + 1;
    const {
      digestAlgorithm: _readinessDigestAlgorithm,
      digest: _readinessDigest,
      ...readinessPayload
    ...
    expect(() =>
      persistVisualContractAuthoringReadiness({
        repoRoot: tempRoot(),
        outputDir: 'outputs/forged-readiness-budget',
        request,
        evidence: forgedReadiness,
        receipt: result.receipt,
        write: false,
      }),
    ).toThrow(/readiness v56 requires exact typed draft-validation evidence/);
  });
            ...
            : args.attempt === 2
              ...
              : args.attempt === 3
                ...
                : (() => {
                    ...
                    const affectedPage = authority.pages.find(
                      (page) => page.pageNumber === independentPage.pageNumber,
                    );
                    const target = affectedPage?.targets.find(
                      (candidate) =>
                        candidate.coverageIndex ===
                        independentPage.actionSemanticCoverage.length - 1,
                    );
                    if (!target) {
                      throw new Error(
  ...
  it('records a bounded pure-structural BookSurface repair without resending action coverage', async () => {
    ...
    expect(result.receipt, JSON.stringify(result.receipt, null, 2)).toMatchObject({
      status: 'completed',
      callCount: 2,
      repairCount: 1,
    });
    ...
    expect(payload.affectedPages).toHaveLength(12);
    expect(secondCall.userPrompt).not.toContain('actionSemanticCoverage');
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/unexpected-fallback',
        request,
        receipt: result.receipt,
    ...
    for (const mutate of [
      (receipt: typeof result.receipt) => {
        receipt.attempts[1]!.systemPromptDigest = '4'.repeat(64);
      },
      (receipt: typeof result.receipt) => {
        receipt.attempts[1]!.inputAccounting.schemaBytes -= 1;
        receipt.attempts[1]!.inputAccounting.estimatedBytes -= 1;
      },
      (receipt: typeof result.receipt) => {
        receipt.attempts[0]!.inputAccounting.userBytes += 1;
        receipt.attempts[0]!.inputAccounting.estimatedBytes += 1;
      },
      (receipt: typeof result.receipt) => {
        receipt.provider = 'forged-provider';
      },
      (receipt: typeof result.receipt) => {
        receipt.endpoint = 'forged-endpoint';
      },
      (receipt: typeof result.receipt) => {
        receipt.model = 'forged-model';
      },
      (receipt: typeof result.receipt) => {
        receipt.serviceTier = 'forged-tier';
      },
      (receipt: typeof result.receipt) => {
        const mutablePricing = receipt.pricing as unknown as {
          uncachedInputUsdPerUnit: number;
        };
        mutablePricing.uncachedInputUsdPerUnit += 1;
        receipt.pricingDigest = canonicalJsonDigest(
          receipt.pricing,
        );
      },
    ]) {
      const replayedReceipt = structuredClone(result.receipt);
      mutate(replayedReceipt);
      const {
        digestAlgorithm: _digestAlgorithm,
        digest: _digest,
        ...receiptPayload
      } = replayedReceipt;
      replayedReceipt.digest = canonicalJsonDigest(receiptPayload);
      expect(
        visualContractAuthoringReceiptPromptAuthorityIsValid({
          request,
      ...
      expect(() =>
        persistVisualContractAuthoringReceipt({
          repoRoot: tempRoot(),
          outputDir: 'outputs/book-surface-prompt-replay',
          request,
          receipt: replayedReceipt,
          write: false,
        }),
    ...
    const malformedAccountingReceipt = structuredClone(
      result.receipt,
    );
    malformedAccountingReceipt.attempts[1]!.inputAccounting.separatorBytes =
      999;
    malformedAccountingReceipt.attempts[1]!.inputAccounting.estimatedBytes +=
      997;
    const {
      digestAlgorithm: _accountingDigestAlgorithm,
      digest: _accountingDigest,
      ...malformedAccountingPayload
    } = malformedAccountingReceipt;
    malformedAccountingReceipt.digest = canonicalJsonDigest(
      malformedAccountingPayload,
    );
    expect(
      visualContractAuthoringReceiptPromptAuthorityIsValid({
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/book-surface-accounting-replay',
        request,
        receipt: malformedAccountingReceipt,
        write: false,
      }),
    ...
    (
      malformedRequest.bookSurfaceRepairStructuredOutput as unknown as {
        strict: boolean;
      }
    ).strict = false;
    const {
    ...
    malformedRequest.digest = canonicalJsonDigest(
      malformedRequestPayload,
    );
    const reboundReceipt = structuredClone(result.receipt);
    reboundReceipt.requestDigest = malformedRequest.digest;
    const {
      digestAlgorithm: _receiptDigestAlgorithm,
      digest: _receiptDigest,
      ...reboundReceiptPayload
    } = reboundReceipt;
    reboundReceipt.digest = canonicalJsonDigest(
      reboundReceiptPayload,
    );
    expect(
      visualContractAuthoringReceiptPromptAuthorityIsValid({
    ...
    const malformedRouteReceipt = structuredClone(result.receipt);
    malformedRouteReceipt.attempts[1]!.repairMode =
      'unknown_repair_mode' as never;
    const {
      digestAlgorithm: _routeReceiptDigestAlgorithm,
      digest: _routeReceiptDigest,
      ...malformedRouteReceiptPayload
    } = malformedRouteReceipt;
    malformedRouteReceipt.digest = canonicalJsonDigest(
      malformedRouteReceiptPayload,
    );
    expect(
      visualContractAuthoringReceiptPromptAuthorityIssues({
        request,
        receipt: malformedRouteReceipt,
      }),
    ).toContain('receipt_attempt_prompt_route_invalid');
  });

  it('reattaches compiler-owned action beat identity after a BookSurface semantic repair and persists a candidate', async () => {
    const snapshot = bunnySnapshot();
    const request = requestFor(snapshot, 'live');
    const valid = fullyActionedBunnyDraft(snapshot);
    for (const page of valid.pageContracts) {
    ...
    const invalid = structuredClone(valid);
    const invalidPage = invalid.pageContracts[0] as unknown as {
      pageNumber: number;
      actionRequirements: Array<Record<string, unknown>>;
    };
    const invalidAction = invalidPage.actionRequirements[0]!;
    const compilerOwnedBeatId = invalidAction.beatId as string;
    invalidAction.laterality = 'left';

    let rewrittenBeatCount = 0;
    const calls: Parameters<VisualContractAuthoringProvider['call']>[0][] = [];
    const provider: VisualContractAuthoringProvider = {
      call: vi.fn(async (args) => {
        calls.push(args);
        ...
        if (args.attempt === 2) {
          const response = bookSurfaceV7Response({
            payload: decodeBookSurfaceRepairUserPrompt(args.userPrompt),
            repaired: {
              coverContract: structuredClone(
                valid.coverContract,
              ) as unknown as Record<string, unknown>,
          ...
          for (const pagePatch of response.pageStructuralPatches as Array<
            Record<string, unknown>
          >) {
            if (!Array.isArray(pagePatch.actionRequirements)) continue;
            for (const action of pagePatch.actionRequirements as Array<
              Record<string, unknown>
            >) {
              action.beatId = `beat:p${pagePatch.pageNumber}:provider_rewrite`;
              rewrittenBeatCount += 1;
            }
          }
          output = response;
        }
        return {
          ...
          receipt: {
            provider: 'openai',
            model: 'gpt-5.6-sol',
            responseId: `action-identity-response-${args.attempt}`,
            usage: {
              input_tokens: 1_000,
              output_tokens: 2_000,
              total_tokens: 3_000,
    ...
    expect(result.receipt.attempts.map((attempt) => attempt.repairMode))
      .toEqual([null, 'book_surface_patch']);
    expect(rewrittenBeatCount).toBeGreaterThan(0);
    expect(calls[1]!.userPrompt).not.toContain('actionSemanticCoverage');
    expect(result.receipt.candidateDigest).toMatch(/^[a-f0-9]{64}$/);
    const compiledPage = result.compileResult!.template.pageContracts[0] as unknown as {
      actionRequirements: Array<Record<string, unknown>>;
    };
    expect(compiledPage.actionRequirements[0]).toMatchObject({
      predicate: 'looks_at',
    });
    expect(
      result.compileResult!.actionSemanticCoverage.find(
        (record) => record.pageNumber === invalidPage.pageNumber,
      ),
    ).toMatchObject({
      beatId: compilerOwnedBeatId,
      disposition: { kind: 'action_requirement' },
    });
  });
  ...
  it('uses the remaining standard repair only for a rejected page action-binding scope and persists the completed three-call lifecycle', async () => {
    ...
    const invalidActions = invalidPage.actionRequirements as Array<
      Record<string, unknown>
    >;
    expect(invalidCoverage).toHaveLength(2);
    ...
    const provider: VisualContractAuthoringProvider = {
      call: vi.fn(async (args) => ({
        ...
        receipt: {
          provider: 'openai',
          model: 'gpt-5.6-sol',
          responseId: `scope-correction-${args.attempt}`,
          usage: {
            input_tokens: 1_000,
            output_tokens: 2_000,
            total_tokens: 3_000,
  ...
  it('keeps an expanded 12-page presentation-gap frontier terminal before BookSurface', async () => {
    ...
    const request = requestFor(snapshot, 'live');
    const fixture = liveShapedAtomicBindingFixture(snapshot);
    const expandedInitial = structuredClone(fixture.initial);
    expandedInitial.recurringProps[0]!.firstRevealPage = 110;
    const expandedPages = expandedInitial.pageContracts as unknown as Array<{
      pageNumber: number;
      actionSemanticCoverage: Array<{
        beatId: string;
        sourceEvidenceId: string;
    ...
    const originalPresentationGapCount = expandedPages
      .flatMap((page) => page.actionSemanticCoverage)
      .filter(
        (coverage) =>
          coverage.disposition.kind === 'unsupported' &&
    ...
    for (let index = 0; index < 13; index += 1) {
      ...
      page.actionSemanticCoverage.push({
        ...structuredClone(coverageSeed),
        beatId: `beat:p${page.pageNumber}:aggregate_gap_${addedPresentationGaps + 1}`,
        disposition: {
          kind: 'unsupported',
          reason: 'closed_action_catalog_gap',
        },
    ...
    const provider: VisualContractAuthoringProvider = {
      call: vi.fn(async (args) => {
        calls.push(args);
        let output: unknown = expandedInitial;
        if (args.attempt > 1) {
          const payload = decodeBookSurfaceRepairUserPrompt(
            args.userPrompt,
          );
        ...
        return {
          ...
          receipt: {
            provider: 'openai',
            model: 'gpt-5.6-sol',
            responseId: `repeated-book-surface-${args.attempt}`,
            usage: {
              input_tokens: 1_000,
              output_tokens: 2_000,
              total_tokens: 3_000,
    ...
    expect(
      result.receipt.attempts.flatMap(
        (attempt) =>
          attempt.draftValidationDiagnostics?.items.map(
            (item) => item.issue.code,
          ) ?? [],
      ),
  ...
  it('records sanitized exact byte accounting when a repair is rejected at input admission', async () => {
    ...
    const firstPage = invalid.pageContracts[0] as unknown as {
      actionSemanticCoverage: Array<Record<string, unknown>>;
    };
    firstPage.actionSemanticCoverage[0]!.disposition = {
      kind: 'non_visual',
      rationale: 'narrative_context',
    };
    ...
    ).toThrow(/readiness v56 requires current/);
  });

  it('stops after a completed repair increases the complete issue census and persists no synthetic attempt', async () => {
    const snapshot = bunnySnapshot();
    const request = requestFor(snapshot, 'live');
    const initial = fullyActionedBunnyDraft(snapshot);
    initial.worldType = '';
    const regressed = fullyActionedBunnyDraft(snapshot);
    ...
    expect(result.receipt).toMatchObject({
      ...
      failure: {
        ...
        diagnosticCodes: ['draft_validation_repair_regressed'],
    ...
    expect(result.receipt.attempts.map((attempt) => attempt.repairMode))
      .toEqual([null, 'full_draft']);
    const previousCount = result.receipt.attempts[0]!
      .draftValidationDiagnostics!.currentUniqueCount;
    const rejectedCount = result.receipt.attempts[1]!
      .draftValidationDiagnostics!.currentUniqueCount;
    expect(rejectedCount).toBeGreaterThan(previousCount);
    expect(result.receipt.attempts[1]!.draftValidationDiagnostics)
      .toMatchObject({ finalAttempt: true });
    expect(result.receipt.failure!.diagnosticCount).toBe(
      Math.min(rejectedCount, 128),
    );
    expect(
      visualContractAuthoringTerminalFailureIsValid(
        result.receipt.failure,
    ...
    expect(readiness.authoringOutcome).toEqual({
      status: 'failed',
      failureCode: 'draft_validation_repair_regressed',
      terminalClassification: result.receipt.failure,
    });
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/repair-regression',
        request,
        receipt: result.receipt,
        write: false,
      }),
    ...
    const forged = structuredClone(result.receipt);
    forged.failure!.diagnosticCount -= 1;
    const {
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/repair-regression-forged',
        request,
        receipt: forged,
        write: false,
      }),
  ...
  it('stops after one completed repair leaves the exact complete issue fingerprint unchanged', async () => {
    const snapshot = bunnySnapshot();
    const request = requestFor(snapshot, 'live');
    const fixedPoint = fullyActionedBunnyDraft(snapshot);
    fixedPoint.worldType = '';
    const provider = sequencedSuccessfulProvider([
      fixedPoint,
      structuredClone(fixedPoint),
      fullyActionedBunnyDraft(snapshot),
    ]);
    ...
    expect(result.receipt).toMatchObject({
      ...
      failure: {
        ...
        diagnosticCodes: ['draft_validation_repair_stagnated'],
        authorityReferenceDiagnostics: null,
        repairOutputDiagnostics: null,
        repairRouteAdmissionDiagnostics: null,
      },
    ...
    expect(result.receipt.attempts.map((attempt) => attempt.repairMode))
      .toEqual([null, 'full_draft']);
    const previousDiagnostics = result.receipt.attempts[0]!
      .draftValidationDiagnostics!;
    const currentDiagnostics = result.receipt.attempts[1]!
      .draftValidationDiagnostics!;
    expect(previousDiagnostics).toMatchObject({
      finalAttempt: false,
      truncated: false,
    });
    expect(currentDiagnostics).toMatchObject({
      finalAttempt: true,
      truncated: false,
      currentUniqueCount: previousDiagnostics.currentUniqueCount,
    });
    expect(result.receipt.failure!.diagnosticCount).toBe(
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/repair-stagnation',
        request,
        receipt: result.receipt,
        write: false,
      }),
    ...
    const forged = structuredClone(result.receipt);
    forged.attempts[1]!.draftValidationDiagnostics!.truncated = true;
    const {
      digestAlgorithm: _forgedDigestAlgorithm,
      digest: _forgedDigest,
      ...forgedPayload
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/repair-stagnation-forged',
        request,
        receipt: forged,
        write: false,
      }),
  ...
  it('records an oversized represented-elsewhere route refusal before a second provider call', async () => {
    const snapshot = bunnySnapshot();
    const request = requestFor(snapshot, 'live');
    const oversizedRepresented =
      withPageEightRepresentedElsewhereResidual(
        fullyActionedBunnyDraft(snapshot),
        `Oversized represented prop-state authority ${'A'.repeat(70_000)}`,
      );
    const provider = successfulProvider(oversizedRepresented);
    ...
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/represented-route-admission-attempt-two',
        request,
        receipt: result.receipt,
        write: false,
      }),
  ...
  it('records a final-standard-slot BookSurface route-admission refusal without inventing a seventh provider attempt', async () => {
    ...
    for (const page of firstAttempt.pageContracts) {
      page.camera = '';
    }
      ...
      call: vi.fn(async (args) => {
        const output =
          args.attempt <= 5
            ...
            : finalSlotMixed;
        if (args.attempt <= 5) {
          output.recurringProps[0]!.description =
            `${output.recurringProps[0]!.description} Attempt ${args.attempt}.`;
        }
        return {
          output: JSON.stringify(output),
          receipt: {
    ...
    representedElsewhereRouteReceipt.failure!
      .repairRouteAdmissionDiagnostics!.repairMode =
      'represented_elsewhere_patch';
    const {
      digestAlgorithm: _representedRouteDigestAlgorithm,
      digest: _representedRouteDigest,
      ...representedRoutePayload
    } = representedElsewhereRouteReceipt;
    representedElsewhereRouteReceipt.digest = canonicalJsonDigest(
      representedRoutePayload,
    );
    expect(
      visualContractAuthoringTerminalFailureIsValid(
    ...
    const representedElsewhereReadiness =
      buildVisualContractAuthoringReadinessEvidence({
        snapshot,
        request,
        receipt: representedElsewhereRouteReceipt,
      });
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/represented-route-admission',
        request,
        receipt: representedElsewhereRouteReceipt,
        write: false,
      }),
    ...
    expect(() =>
      persistVisualContractAuthoringReadiness({
        ...
        outputDir: 'outputs/represented-route-admission',
        request,
        evidence: representedElsewhereReadiness,
        receipt: representedElsewhereRouteReceipt,
        write: false,
      }),
    ).not.toThrow();

    const representedRoutePositionTamper = structuredClone(
      representedElsewhereRouteReceipt,
    );
    representedRoutePositionTamper.failure!
      .repairRouteAdmissionDiagnostics!.repairAttempt = 6;
    const {
      digestAlgorithm: _representedPositionDigestAlgorithm,
      digest: _representedPositionDigest,
      ...representedPositionPayload
    } = representedRoutePositionTamper;
    representedRoutePositionTamper.digest = canonicalJsonDigest(
      representedPositionPayload,
    );
    expect(() =>
      persistVisualContractAuthoringReceipt({
        repoRoot: tempRoot(),
        outputDir: 'outputs/represented-route-position-tamper',
        request,
        receipt: representedRoutePositionTamper,
        write: false,
      }),
    ).toThrow(/receipt v59 requires/);
  ...
  it('records a bounded book-surface repair without resending unrelated global draft authority', async () => {
    ...
    if (!coverZone) throw new Error('missing cover zone fixture');
    valid.coverContract.zoneId = coverZone.id;
    valid.coverContract.castIds = [
      valid.cast.child.id,
      ...(valid.cast.companion ? [valid.cast.companion.id] : []),
    ...
    const invalid = structuredClone(valid);
    invalid.coverContract.mustShow = [''];
    const invalidPage = invalid.pageContracts[0]! as PageVisualContract & {
      actionSemanticCoverage: Array<{
        beatId: string;
        sourceEvidenceId: string;
        disposition: Record<string, unknown>;
    ...
    delete (repairedPage as unknown as Record<string, unknown>).castIds;
    ...
    expect(repairPayload).not.toHaveProperty('storySource');
    expect(repairPayload).not.toHaveProperty('previousDraft');
    expect(
      (repairPayload.affectedPages as Array<{ pageNumber: number }>).map(
    ...
    const invalidRepairProvider: VisualContractAuthoringProvider = {
      call: vi.fn(async (args) => {
        let output: unknown = invalid;
        if (args.attempt !== 1) {
          const repair = bookSurfaceV7Response({
            payload: decodeBookSurfaceRepairUserPrompt(
              args.userPrompt,
            ),
            repaired: {
    ...
    const legacyReceipt = structuredClone(rejected.receipt);
    const legacyRepairDiagnostics = legacyReceipt.failure
      ?.repairOutputDiagnostics as unknown as Record<string, unknown> | null;
    if (!legacyRepairDiagnostics) {
      throw new Error('missing legacy repair diagnostics fixture');
    }
    legacyRepairDiagnostics.version =
      'visual-contract-repair-output-diagnostics/v1';
    delete legacyRepairDiagnostics.targetContext;
    const {
      digestAlgorithm: _legacyDigestAlgorithm,
      digest: _legacyDigest,
      ...legacyReceiptPayload
    ...
    const legacyReceiptWrite = persistVisualContractAuthoringReceipt({
      repoRoot: rejectedRoot,
      outputDir: 'outputs/book-surface-repair-identity-legacy',
      request: rejectedRequest,
      receipt: legacyReceipt,
      write: true,
    });
    const legacyReadinessWrite = persistVisualContractAuthoringReadiness({
      repoRoot: rejectedRoot,
      outputDir: 'outputs/book-surface-repair-identity-legacy',
      request: rejectedRequest,
      evidence: legacyReadiness,
      receipt: legacyReceipt,
      write: true,
    });
    const loadedLegacyReceipt = JSON.parse(
      fs.readFileSync(
        path.join(rejectedRoot, legacyReceiptWrite.path),
        'utf8',
      ),
    ) as typeof legacyReceipt;
    const loadedLegacyReadiness = JSON.parse(
      fs.readFileSync(
        path.join(rejectedRoot, legacyReadinessWrite.path),
        'utf8',
      ),
    ) as typeof legacyReadiness;
    expect(
      loadedLegacyReceipt.failure?.repairOutputDiagnostics?.version,
    ).toBe('visual-contract-repair-output-diagnostics/v1');
    expect(
      visualContractAuthoringTerminalFailureIsValid(
        loadedLegacyReceipt.failure,
      ),
    ).toBe(true);
    expect(
      loadedLegacyReadiness.authoringOutcome.terminalClassification,
    ).toEqual(loadedLegacyReceipt.failure);
    expect(
      buildVisualContractAuthoringReadinessEvidence({
        snapshot,
        request: rejectedRequest,
        receipt: loadedLegacyReceipt,
      }),
  ...
  it('keeps a non-BookSurface mixed authority failure terminal when presentation eligibility is absent', async () => {
    ...
    (
      intermediate.pageContracts[0]!.actionRequirements![0] as unknown as Record<
        string,
        unknown
      >
    ).object = {
