...
describe('exact zero-cost authoring preflight', () => {
  ...
  it('rejects a forged request_invalid zero-attempt receipt that launders the invalid request schedule', async () => {
    ...
    predecessorRequest.version =
      'visual-contract-authoring-request/v32' as typeof predecessorRequest.version;
    const {
  ...
  it.each([13, 17, 23])(
    ...
    expect(() =>
      ...
      /readiness v35 requires exact typed draft-validation evidence/,
    );
  });
...
describe('sanitized receipts and immutable artifact lifecycle', () => {
  it('attempts only the compact presentation lane for homogeneous catalog gaps and fails closed on unusable repair output', async () => {
    ...
    expect(readiness).toMatchObject({
      version: 'visual-contract-authoring-readiness/v35',
      draftValidation: {
        status: 'interrupted',
  ...
  it('classifies every prior authoring authority as immutable without mutating its bytes', () => {
    const immediatelyPrior = [
      ['request', 'visual-contract-authoring-request/v32'],
      ['receipt', 'visual-contract-authoring-receipt/v36'],
      ['readiness', 'visual-contract-authoring-readiness/v34'],
  ...
  it('rejects the immediately prior receipt as current readiness authority after canonical redigest', async () => {
    ...
    prior.version = 'visual-contract-authoring-receipt/v36';
    const {
      digestAlgorithm: _digestAlgorithm,
      digest: _digest,
      ...payload
  ...
  it('rejects the immediately prior readiness as current authority after canonical redigest', async () => {
    ...
    const prior = structuredClone(readiness) as unknown as Record<
      ...
      unknown
    >;
    prior.version = 'visual-contract-authoring-readiness/v34';
    const {
      digestAlgorithm: _digestAlgorithm,
      digest: _digest,
      ...payload
    ...
    ).toThrow(/readiness v35 requires/);
  });
  ...
  it('rejects persistence of re-digested exhaustion receipt evidence bound to only one logical call and zero repairs', async () => {
    ...
    expect(() =>
      ...
      /receipt v37 requires exact typed draft-validation evidence/,
    );
  });
  ...
  it('persists a fourth-call terminal reference cleanup and rejects budget-sequence tampering', async () => {
    ...
    ).toThrow(/readiness v35 requires exact typed draft-validation evidence/);
  });
  ...
  it('uses the remaining standard repair only for a rejected page action-binding scope and persists the completed three-call lifecycle', async () => {
    ...
    expect(result.receipt).toMatchObject({
      version: 'visual-contract-authoring-receipt/v37',
      status: 'completed',
      callCount: 3,
      repairCount: 2,
      draftValidationStatus: 'completed',
  ...
  it('records sanitized exact byte accounting when a repair is rejected at input admission', async () => {
    ...
    ).toThrow(/readiness v35 requires current/);
  });
