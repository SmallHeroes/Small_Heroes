...
describe('exact zero-cost authoring preflight', () => {
  ...
  it('rejects a forged request_invalid zero-attempt receipt that launders the invalid request schedule', async () => {
    ...
    predecessorRequest.version =
      'visual-contract-authoring-request/v30' as typeof predecessorRequest.version;
    const {
  ...
  it.each([13, 17, 23])(
    ...
    expect(() =>
      ...
      /readiness v33 requires exact typed draft-validation evidence/,
    );
  });
...
describe('sanitized receipts and immutable artifact lifecycle', () => {
  ...
  it('classifies every prior authoring authority as immutable without mutating its bytes', () => {
    const immediatelyPrior = [
      ['request', 'visual-contract-authoring-request/v30'],
      ['receipt', 'visual-contract-authoring-receipt/v34'],
      ['readiness', 'visual-contract-authoring-readiness/v32'],
  ...
  it('rejects the immediately prior receipt as current readiness authority after canonical redigest', async () => {
    ...
    prior.version = 'visual-contract-authoring-receipt/v34';
    const {
      digestAlgorithm: _digestAlgorithm,
      digest: _digest,
      ...payload
  ...
  it('rejects the immediately prior readiness as current authority after canonical redigest', async () => {
    ...
    const prior = structuredClone(readiness) as unknown as Record<
      ...
      unknown
    >;
    prior.version = 'visual-contract-authoring-readiness/v32';
    const {
      digestAlgorithm: _digestAlgorithm,
      digest: _digest,
      ...payload
    ...
    ).toThrow(/readiness v33 requires/);
  });
  ...
  it('rejects persistence of re-digested exhaustion receipt evidence bound to only one logical call and zero repairs', async () => {
    ...
    expect(() =>
      ...
      /receipt v35 requires exact typed draft-validation evidence/,
    );
  });
  ...
  it('persists a fourth-call terminal reference cleanup and rejects budget-sequence tampering', async () => {
    ...
    ).toThrow(/readiness v33 requires exact typed draft-validation evidence/);
  });
  ...
  it('repeats BookSurface with null cover for the 12-page pure structural residual and persists a candidate', async () => {
    ...
    const provider: VisualContractAuthoringProvider = {
      call: vi.fn(async (args) => {
        ...
        if (args.attempt > 1) {
          ...
          const response = bookSurfaceV5Response({
            payload,
            repaired:
              args.attempt === 2
                ? firstRepair
    ...
    const coverHintCount = (
      mixedPayload.coverAuthority as { validationHints: unknown[] }
  ...
  it('records sanitized exact byte accounting when a repair is rejected at input admission', async () => {
    ...
    ).toThrow(/readiness v33 requires current/);
  });
  ...
  it('records a bounded book-surface repair without resending unrelated global draft authority', async () => {
    ...
    const invalidRepairProvider: VisualContractAuthoringProvider = {
      call: vi.fn(async (args) => {
        ...
        if (args.attempt !== 1) {
          const repair = bookSurfaceV5Response({
            payload: decodeBookSurfaceRepairUserPrompt(
              args.userPrompt,
            ),
            repaired: {
