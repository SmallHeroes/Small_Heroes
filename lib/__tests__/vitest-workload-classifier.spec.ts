...
describe('Vitest workload classifier', () => {
  ...
  it('creates one total disjoint partition and defaults new specs to ordinary', () => {
    ...
    expect(partition.inventory).toHaveLength(389);
    ...
    expect(partition.ordinary).toHaveLength(367);
    ...
    expect(partition.ordinary).toContain('lib/__tests__/action-semantic-catalog-wire-policy.spec.ts');
    expect(partition.ordinary).toContain(
      'lib/__tests__/supporting-cast-review.spec.ts',
    );
    expect(partition.ordinary).toContain(
      'lib/visual-package/__tests__/accepted-supporting-cast-review.spec.ts',
    );
    ...
    expect(partition.ordinary).toContain(
      'lib/visual-package/__tests__/story-source-visual-direction-correction-batch.spec.ts',
    );
    expect(partition.resourceIntensive).toContain(
      'lib/__tests__/story-source-visual-direction-correction-acceptance-lifecycle.spec.ts',
    );
    expect(partition.ordinary).toContain(
