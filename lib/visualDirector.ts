...
export function resolveStyleSentence(selectedStyle: string): string {
  ...
  if (styleId === STYLE_IDS.DETAILED_WHIMSICAL_WORLD) {
    return "Dense hand-drawn illustrated world: visible ink outlines, sketchy crosshatching, imperfect contour lines, warm gouache/watercolor shading, rich layered textures. Halfway between realistic and cartoon. Character is 30-50% of image, integrated into dense cozy environment with many hidden visual moments. Warm amber lighting with cool blue shadows. Upper area softer and lower contrast but still illustrated. No glossy AI realism, no clean vector, no flat cartoon, no empty spaces.";
  }
