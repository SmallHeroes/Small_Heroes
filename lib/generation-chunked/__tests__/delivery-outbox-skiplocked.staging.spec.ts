...
describe.skipIf(!RUN)('delivery-outbox SKIP LOCKED — staging real DB (Supavisor txn pooling)', () => {
  it('two concurrent claimers split the seed rows exactly: union == 4, intersection == ∅', async () => {
    ...
    try {
      const now = new Date();
      for (let i = 0; i < 4; i++) {
        await enqueueDelivery(prisma, { orderId: `${marker}-${i}`, scope: 'base_book', fulfillmentVersion: 1, manifestId: `${marker}-m`, inputVersion: 0, deliveryFenceVersion: 0, payload, now });
      }
